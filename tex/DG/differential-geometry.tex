...
\begin{topic}{chern-class}{Chern class}
    ...
    where $c_k(E) \in \Omega^{2k}(M)$ is the \textbf{$k$-th Chern form}. The Chern forms are \tref{closed-form}{closed}, and the cohomology class $[c_k(E)] \in H^{2k}(M)$ is called the \textbf{$k$-th Chern class} of $E$. These classes are independent on the choice of connection.
\end{topic}
...
\begin{topic}{chern-character}{Chern character}
    ...
    The cohomology class $[\textup{ch}(E)] \in H^\bdot(M)$ is independent on the choice of connection.
...
\begin{topic}{dolbeault-operators}{Dolbeault operators}
    Let $M$ be a complex manifold with coordinates $z^1, \ldots, z^n$. The \textbf{Dolbeault operators} are the operators
    \[ \partial = \sum_{j = 1}^{n} dz^j \wedge \frac{\partial}{\partial z^j} \quad \text{and} \quad \bar{\partial} = \sum_{j = 1}^{n} d\bar{z}_j \wedge \frac{\partial}{\partial \bar{z}_j} . \]
