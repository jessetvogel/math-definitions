...
\begin{example}{fourier-transform-finite-groups}
    The \textit{convolution} of two functions $f_1, f_2 \in C(G)$ is defined by
    ...
    \[ \mathcal{F}(f_1 * f_2) = \mathcal{F}(f_1) \cdot \mathcal{F}(f_2) \quad \textup{ and } \quad \widehat{\mathcal{F}}(F_1 \cdot F_2) \cdot |G| = \widehat{\mathcal{F}}(F_1) * \widehat{\mathcal{F}}(F_2) . \]
\end{example}