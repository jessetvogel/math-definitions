...
\begin{topic}{spectrum}{spectrum}
    A \textbf{spectrum} is a sequence of pointed \tref{topological-space}{topological spaces} $E = (E_n)_{n \ge 0}$ equipped with \tref{weak-homotopy-equivalence}{weak homotopy equivalences} $E_n \xrightarrow{\sim} \Omega E_{n + 1}$, where $\Omega E_{n + 1}$ denotes the \tref{loop-space}{loop space} of $E_{n + 1}$.
\end{topic}
