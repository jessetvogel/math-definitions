...
\begin{example}{cw-complex}
    The surface of a cube has the structure of an absolute CW-complex where the $0$-cells are the $8$ vertices, the $1$-cells are the $12$ edges, and the $2$-cells are the $6$ faces.

    The $n$-sphere $S^n$ has the structure of an absolute CW-complex with one $0$-cell and one $n$-cell.
    
    The real projective space $\PP_\RR^n$ has the structure of an absolute CW-complex having a single $k$-cell for each $0 \le k \le n$.
\end{example}
