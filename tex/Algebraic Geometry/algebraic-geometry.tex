...
\begin{topic}{ringed-space}{(locally) ringed space}
    ...
    A morphism of ringed spaces from $(X, \mathcal{O}_X)$ to $(Y, \mathcal{O}_Y)$ is a pair $(f, f^\#)$ of a continuous map $f \colon X \to Y$ and a map $f^\# \colon \mathcal{O}_Y f_* \mathcal{O}_X$ of sheaves of rings on $Y$.
    ...
    A morphism of locally ringed spaces is a morphism $(f, f^\#)$ of ringed spaces such that for each point $x \in X$ the induced map of local rings $f^\#_x \colon \mathcal{O}_{Y, f(x)} \mathcal{O}_{X, x}$ is a \textit{local morphism}, i.e. the pre-image of the maximal ideal of $\mathcal{O}_{X, x}$ is the maximal ideal of $\mathcal{O}_{Y, f(x)}$.
\end{topic}
...
\begin{topic}{finite-type}{(locally) of finite type}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{of finite type at $x \in X$} if there exist affine opens $U = \Spec A \subset X$ containing $x$ and $V = \Spec B \subset Y$ with $f(U) \subset V$ such that $A$ is a \tref{AA:finitely-generated-algebra}{finitely generated} $B$-algebra (via the induced map $B \to A$).
...
\begin{topic}{finite-presentation}{(locally) of finite presentation}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{of finite presentation at $x \in X$} if there exists affine opens $U = \Spec A \subset X$ containing $x$ and $V = \Spec B \subset Y$ with $f(U) \subset V$ such that $A$ is a \tref{AA:finitely-presented-algebra}{finitely presented} $B$-algebra (via the induced map $B \to A$).
...
\begin{topic}{integral-morphism}{integral morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{integral} if it is \tref{affine-morphism}{affine} and for every affine open $\Spec B = V \subseteq Y$ with inverse image $\Spec A = f^{-1}(V) \subseteq X$ the corresponding ring morphism $B \to A$ is \tref{AA:integral-closure}{integral}.
\end{topic}

\begin{topic}{radical-morphism}{radical morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{radical} if it is injective (on the level of topological spaces) and for every $x \in X$ the \tref{AA:field-extension}{field extension} $\kappa(x) / \kappa(f(x))$ is \tref{AA:purely-inseparable-field-extension}{purely inseparable}.
\end{topic}

\begin{topic}{finite-morphism}{finite morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{finite} if there exists a covering of $Y$ by open affine subsets $V_i = \Spec B_i$, such that for each $i$, $f^{-1}(V_i)$ is affine, equal to $\Spec A_i$, where $A_i$ is a $B_i$-algebra which is a \tref{AA:finitely-generated-module}{finitely generated} $B_i$-module.
\end{topic}

\begin{topic}{quasi-finite-morphism}{quasi-finite morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{quasi-finite} if it is of \tref{finite-type}{finite type}, and every point $x \in X$ is isolated in its fiber $f^{-1}(f(x))$, i.e. every fiber is a discrete finite set.
\end{topic}
...
\begin{example}{quasi-finite-morphism}
    ...
    \[ i \colon \Spec \ZZ[\tfrac{1}{2}] \to \Spec \ZZ . \]
...
\begin{topic}{open-immersion}{open immersion}
    An \textbf{open immersion} is a morphism $j \colon U \to X$ of \tref{scheme}{schemes} which induces an isomorphism of $U$ with an open subscheme of $X$.
\end{topic}

\begin{topic}{closed-immersion}{closed immersion}
    A \textbf{closed immersion} is a morphism $i \colon Z \to X$ of \tref{scheme}{schemes} such that $i$ induces a \tref{TO:homeomorphism}{homeomorphism} of the underlying space of $Z$ onto a closed subset of that of $X$, and furthermore the induced map $i^\# \colon \mathcal{O}_X i_*\mathcal{O}_Z$ of sheaves on $X$ is surjective.
\end{topic}
...
\begin{topic}{affine-morphism}{affine morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{affine} if the inverse image of every affine open in $Y$ is an affine open of $X$. 
\end{topic}

\begin{example}{affine-morphism}
    \begin{itemize}
        \item Any morphism $f \colon \Spec(R) \to \Spec(S)$ between affine schemes is affine.
        \item The inclusion $\iota \colon \AA^2_k \setminus \{ (0, 0) \} \to \AA^2_k$ is not affine, since $\AA^2_k \setminus \{ (0, 0) \}$ is not affine.
        \item Let $Y_1 = Y_2 = \AA^2_k$, and let $Y$ be obtained by gluing $Y_1$ and $Y_2$ along $\AA^2_k \setminus \{ (0, 0) \}$. Then the inclusions $j_i \colon Y_i \to Y$ are not affine, since $j_1^{-1}(Y_2) = j_2^{-1}(Y_1) = \AA^2_k \setminus \{ (0, 0) \}$ is not affine.
    \end{itemize}
\end{example}

\begin{topic}{quasi-compact-morphism}{quasi-compact morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{quasi-compact} if the inverse image of every \tref{TO:compact-space}{compact} open in $Y$ is compact in $X$.
\end{topic}
...
\begin{topic}{separated-morphism}{(quasi-)separated morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{quasi-separated} if the diagonal $\Delta_{X/Y} \colon X \to X \times_Y X$ is quasi-compact, and it is \textbf{separated} if the diagonal $\Delta_{X/Y}$ is a closed immersion.
\end{topic}

\begin{example}{separated-morphism}
    Any morphism of affine schemes is separated. Namely, for any $f \colon \Spec A \to \Spec B$, the diagonal $\Delta$ corresponds to the ring map $A \otimes_B A \to A : a \otimes a' \mapsto aa'$, which is clearly surjective. In this way $A$ is seen as a quotient of $A \otimes_B A$, so $\Delta$ is a closed immersion.
\end{example}
...
\begin{topic}{proper-morphism}{proper morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{proper} if it is \tref{separated-morphism}{separated}, \tref{finite-type}{of finite type} and \tref{TO:universally-closed}{universally closed}.
\end{topic}

\begin{example}{proper-morphism}
    Any \tref{closed-immersion}{closed immersion} $i \colon Y \to X$ is proper. Indeed, the base change of a closed immersion is again a closed immersion, so $i$ is universally closed. Separatedness can be checked locally on the target: if $X = \Spec R$ then $Y = \Spec R / I$ for some ideal $I \subset R$, so $Y$ is also affine, and any morphism between affine schemes is separated. From this local expression of $X$ and $Y$ it is also clear $i$ is of finite type.
\end{example}

\begin{example}{proper-morphism}
    The morphism $\AA_k^1 \to \Spec k$ is not proper as it is not universally closed. Namely, the projection $\AA^1_k \times \AA^1_k \to \AA^1_k$ sends the closed set $\{ xy - 1 = 0 \}$ to $\AA^1_k \setminus \{ 0 \}$ and thus cannot be a closed mapping. However, the morphism is clearly of finite type, and also separated as the diagonal $\Delta \colon \AA^1_k \to \AA^1_k \times \AA^1_k$ is closed, it is given by $\{ x - y = 0 \}$.
\end{example}

\begin{example}{proper-morphism}
    Let $X$ be the affine line with double origin, i.e. two copies of $\AA^1$ glued along $\AA^1 \setminus \{ 0 \}$. Then $X \to \AA^1$ is of finite type and universally closed (the latter as $X_R \to \AA^1_R$ is closed for all rings $R$). However, $X \to \AA^1$ is not separated and hence not proper. For this we need to show $\Delta_{X / \AA^1} \colon X \to X \times_{\AA^1} X$ is not a closed immersion. Consider $\AA^1 \to X \times_{\AA^1} X$ induced from the two maps different maps $\AA^1 \to X$, and note that the inverse image of the diagonal is $\AA^1 \setminus \{ 0 \}$ which is not closed. Hence the diagonal is not closed.
\end{example}

\begin{topic}{projective-morphism}{(quasi-)projective morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is
    \begin{itemize}
...
\begin{topic}{smooth-morphism}{smooth morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{smooth} if
    \begin{itemize}
...
\begin{topic}{unramified-morphism}{unramified morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{unramified} if
    \begin{itemize}
...
\begin{topic}{etale-morphism}{étale morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{étale} if it is \tref{flat-morphism}{flat}, \tref{unramified-morphism}{unramified} and \tref{finite-presentation}{locally of finite presentation}.
\end{topic}

\begin{topic}{formally-smooth}{formally smooth morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{formally smooth} if for every ring $A$, ideal $I \subset A$ with $I^2 = 0$, and commutative diagram
    \[ \svg \begin{tikzcd} \Spec A/I \arrow{r} \arrow{d} & X \arrow{d}{f} \\ \Spec A \arrow{r} \arrow[dashed]{ur} & Y \end{tikzcd} \]
...
\begin{topic}{formally-unramified}{formally unramified morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{formally unramified} if for every ring $A$, ideal $I \subset A$ with $I^2 = 0$, and commutative diagram
    \[ \svg \begin{tikzcd} \Spec A/I \arrow{r} \arrow{d} & X \arrow{d}{f} \\ \Spec A \arrow{r} \arrow[dashed]{ur} & Y \end{tikzcd} \]
...
\begin{topic}{formally-etale}{formally étale morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{formally étale} if for every ring $A$, ideal $I \subset A$ with $I^2 = 0$, and commutative diagram
    \[ \svg \begin{tikzcd} \Spec A/I \arrow{r} \arrow{d} & X \arrow{d}{f} \\ \Spec A \arrow{r} \arrow[dashed]{ur} & Y \end{tikzcd} \]
...
\begin{example}{formally-etale}
    Any \tref{etale-morphism}{étale morphism} is formally étale, but the converse does not hold. Namely, the morphism $f \colon \Spec k \to \Spec k[x_1, x_2, \ldots] / (x_1^2, x_2^2, \cdots)$ is easily checked to be formally étale. However, since $f$ is not \tref{finite-presentation}{locally of finite presentation}, it cannot be étale.
...
\begin{topic}{flat-morphism}{flat morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{flat} if for every $x \in X$, the local ring $\mathcal{O}_{X, x}$ is \tref{AA:flat-module}{flat} as an $\mathcal{O}_{Y, f(x)}$-module.
\end{topic}
...
\begin{topic}{dominant-morphism}{dominant morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{dominant} if the image of $f$ is a \tref{TO:dense-set}{dense} subset of $Y$.
\end{topic}
...
\begin{topic}{picard-functor}{Picard functor}
    Let $f \colon X \to S$ be a \tref{scheme}{scheme} over $S$. The \textbf{absolute Picard functor} is the functor that assigns to any $T \to S$ the \tref{picard-group}{Picard group} $\textup{Pic}(X \times_S T)$,
    \[ \textup{Pic}_X \colon \textbf{Sch}/S \to \textbf{Ab}, \quad T \mapsto \textup{Pic}(X \times_S T) . \]
    The \textbf{relative Picard functor} is the functor
    \[ \textup{Pic}_{X/S} \colon \textbf{Sch}/S \to \textbf{Ab}, \quad T \mapsto \textup{Pic}(X \times_S T) / f_T^* \textup{Pic}(T) . \]
\end{topic}
...
\begin{topic}{fppf-covering}{fppf covering}
    An \textbf{fppf covering} of a \tref{scheme}{scheme} $X$ is a family of morphisms $\left\{ f_i \colon X_i \to X \right\}_{i \in I}$ that are all \tref{flat-morphism}{flat}, \tref{finite-presentation}{locally of finite presentation}, and jointly surjective, i.e. $X = \bigcup_{i \in I} f_i(X_i)$.
...
\begin{topic}{fpqc-covering}{fpqc covering}
    An \textbf{fpqc covering} of a \tref{scheme}{scheme} $X$ is a family of morphisms $\left\{ f_i \colon X_i \to X \right\}_{i \in I}$ that are all \tref{flat-morphism}{flat}, and such that for every affine open $U \subset X$ there exists a finite subset $J \subset I$ and affine opens $V_j \subset X_j$ for each $j \in J$ such that $U = \bigcup_{j \in J} f_j(V_j)$. That is, every affine open in $X$ can be covered by finitely many affine opens from the $X_i$.
    
    `fpqc' stands for \textit{fidèlement plat et quasi-compact}.
\end{topic}

\begin{example}{fpqc-covering}
    Any \tref{fppf-covering}{fppf covering} $\{ f_i \colon X_i \to X \}_{i \in I}$ is an fpqc covering. By assumption the $f_i$ are flat. Let $U \subset X$ be an affine open. Write $f_i^{-1}(U) = \cup_{j \in J_i} U_{ij}$ for some affine opens $U_{ij} \subset X_i$. Each $f_i$ flat and locally of finite presentation, hence open. So, $U = \cup_{i \in I} \cup_{j \in J_i} f_i(U_{ij})$ is an open cover for $U$. Now, $U$ is affine and thus quasi-compact, so there is a finite subcover.
\end{example}
...
\begin{example}{fpqc-covering}
    ...
    \[ f \colon \bigsqcup_{x \in X} \Spec(\mathcal{O}_{X, x}) \to X . \]
...
\begin{topic}{faithfully-flat-morphism}{faithfully flat morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{faithfully flat} if it is both \tref{flat-morphism}{flat} and surjective.
\end{topic}
...
\begin{topic}{projection-formula}{projection formula}
    Let $f \colon X \to Y$ be a morphism of \tref{scheme}{schemes}, $\mathcal{F}$ an $\mathcal{O}_X$-module, and $\mathcal{E}$ a \tref{free-sheaf}{locally free} $\mathcal{O}_Y$-module. The \textbf{projection formula} states a natural isomorphism
...
\begin{topic}{rational-map}{rational map}
    A \textbf{rational map} $f \colon X \dashrightarrow Y$ between (\tref{TO:irreducible-space}{irreducible}) \tref{variety}{varieties} $X$ and $Y$ is an equivalence class of pairs $(U, f_U)$, where $U \subset X$ open and non-empty, $f_U \colon U \to Y$ a morphism, and where two pairs $(U, f_U)$ and $(V, f_V)$ are equivalent if their restriction to $U \cap V$ coincides.
\end{topic}

\begin{topic}{birational-map}{birational map}
    A \textbf{birational map} between (\tref{TO:irreducible-space}{irreducible}) \tref{variety}{varieties} $X$ and $Y$ is a \tref{rational-map}{rational map} $f \colon X \dashrightarrow Y$ which has a rational inverse. That is, there exists a rational map $g \colon Y \dashrightarrow X$ such that $gf = \id_X$ and $fg = \id_Y$ as rational maps.
...
\begin{example}{birational-map}
    ...
    \[ f \colon \AA^1 \dashrightarrow X, \quad t \mapsto \left(\frac{2t}{1 + t^2}, \frac{1 - t^2}{1 + t^2}\right) \]
    has a rational inverse
    \[ g \colon X \dashrightarrow \AA^1, \quad (x, y) \mapsto \frac{1 - y}{x} . \]
\end{example}

\begin{example}{birational-map}
    Affine space $\AA^n$ is birational to projective space $\PP^n$ since the inclusion
    \[ i \colon \AA^n \to \PP^n, \quad (x_1, x_2, \ldots, x_n) \mapsto (1 : x_1 : \cdots : x_n) \]
    has rational inverse
    \[ j \colon \PP^n \dashrightarrow \AA^n, \quad (X_0 : X_1 : \cdots : X_n) \mapsto \left(\frac{X_1}{X_0}, \frac{X_2}{X_0}, \cdots, \frac{X_n}{X_0}\right) . \]
\end{example}
...
\begin{topic}{syntomic-morphism}{syntomic morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{syntomic at $x \in X$} if there exists an affine open neighborhood $U = \Spec A \subset X$ of $x$ and an affine open $V = \Spec B \subset Y$ with $f(U) \subset V$ such that the induced ring morphism $B \to A$ is \textit{syntomic}, i.e. of \tref{AA:finitely-presented-algebra}{finite presentation}, makes $A$ into a \tref{AA:flat-module}{flat} \tref{AA:module}{$B$-module}, and all the fiber rings $A \otimes_B k(\mathfrak{p})$ are local complete intersections.
    
    The morphism $f$ is \textbf{syntomic} if it is syntomic at all points $x \in X$.
\end{topic}

\begin{topic}{galois-cover}{Galois cover}
    A morphism $f \colon X \to S$ of \tref{scheme}{schemes} is a \textbf{Galois cover} if it is \tref{finite-morphism}{finite}, \tref{faithfully-flat-morphism}{faithfully flat}, and such that there exists a finite group $\Gamma$ of $S$-automorphisms, such that
...
\begin{example}{galois-cover}
    Let $n \ge 1$ and let $k$ be a field containing a primitive $n$-th root unity $\zeta_n \in k$. Consider $f \colon X \to S$ with $X = S = \GG_m = \Spec k[x, x^{-1}]$ given by $x \mapsto x^n$. Then we see that
...
\begin{example}{galois-cover}
    Let $E$ be an elliptic curve over $k$, and $n$ an integer invertible in $k$. Consider the map $[n] \colon E \to E$ given by multiplication by $n$. We have an isomorphism
...
\begin{topic}{equivariant-sheaf}{equivariant sheaf}
    Given a \tref{scheme}{scheme} $X$ with an action $\sigma \colon G \times X \to X$ of a group scheme $G$, an \textbf{equivariant sheaf} on $X$ is an \tref{O-module}{$\mathcal{O}_X$-module} $\mathcal{F}$ together with an isomorphism of sheaves on $G \times X$,
    \[ \phi \colon \sigma^* \mathcal{F} \xrightarrow{\sim} \pi_2^* \mathcal{F} \]
    satisfying the \textit{cocycle condition}
    \[ \pi_{23}^* \phi \circ (\id_G \times \sigma)^* \phi = (m \times \id_X)^* \phi \]
    on $G \times G \times X$, where $m \colon G \times G \to G$ is the multiplication map on $G$. That is, the following diagram commutes:
    \[ \svg \begin{tikzcd}[column sep=5em]
...
\begin{topic}{group-scheme}{group scheme}
    Let $S$ be a \tref{scheme}{scheme}. A \textbf{group scheme} over $S$ is a \tref{CT:group-object}{group object} in the category of schemes over $S$. That is, a scheme $G$ over $S$ together with morphisms $m \colon G \times G \to G$ (\textit{multiplication}), $e \colon 1 \to G$ (\textit{group unit}) and $i \colon G \to G$ (\textit{inversion}), such that
    \begin{itemize}
...
\begin{topic}{weil-divisor}{Weil divisor}
    ...
    where the sum is taken over all prime divisors $Y$, and $v_Y \colon K(X) \to \ZZ$ denotes the discrete valuation of $Y$. Any Weil divisor of this form is called a \textbf{principal Weil divisor}.
...
\begin{topic}{segre-embedding}{Segre embedding}
    ...
    \[ \begin{aligned}
        \sigma \colon \PP^n \times \PP^m &\to \PP^{(n + 1)(m + 1) - 1} \\
...
\begin{example}{toric-variety}
    ...
    Conversely, given a toric variety $X$ with torus $T$, one construct a fan $\Delta \subset N_\RR$ with $N = \Hom(\GG_m, T)$ the lattice of one-parameter subgroups and $M = \Hom(T, \GG_m)$ the character lattice of $T$. For any one-parameter subgroup $\lambda \colon \GG_m \to T$, denote by $\lim_{t \to 0} \lambda(t)$ the limit point in $X$ if it exists. Partitioning $N_\RR$ based on the limit points yields a fan $\Delta$ (after taking closures), such that the corresponding toric variety $X(\Delta)$ is isomorphic to $X$.
...
\begin{topic}{algebraic-space}{algebraic space}
    ...
    \[ F \colon (\textbf{Sch}/S)_\textup{fppf}^\textup{op} \to \textbf{Set} \]
    such that
    \begin{itemize}
        \item $F$ is a \tref{CT:sheaf}{sheaf},
        \item the diagonal $\Delta \colon F \to F \times F$ is \textit{representable}, that is, for every $T \in (\textbf{Sch}/S)_\textup{fppf}$ and $\xi \in (F \times F)(T) = F(T) \times F(T)$ the fiber product $T \times_{(F \times F)} F$ is representable by a scheme,
...
\begin{topic}{ample-invertible-sheaf}{(very) ample invertible sheaf}
    Let $X$ be a \tref{scheme}{scheme} over $S$. An \tref{invertible-sheaf}{invertible sheaf} $\mathcal{L}$ on $X$ is called \textbf{very ample} relative to $S$, if there is an \tref{immersion}{immersion} $i \colon X \to \PP_S^n$ for some $n$, such that $\mathcal{L} \isom i^*(\mathcal{O}_{\PP_S^n}(1))$.
...
\begin{topic}{cech-cohomology}{Čech cohomology}
    ...
    with differential
    \[ d^p \colon C^p(\mathcal{U}, \mathcal{F}) \to C^{p + 1}(\mathcal{U}, \mathcal{F}) \]
...
\begin{example}{cech-cohomology}
    Note that 
    \[ d^0 \colon C^0(\mathcal{U}, \mathcal{F}) = \prod_{i \in I} \mathcal{F}(U_i) \to C^1(\mathcal{U}, \mathcal{F}) = \prod_{i < j} \mathcal{F}(U_{ij}) \]
...
\begin{topic}{regular-morphism}{regular morphism}
    Let $f \colon X \to Y$ be a morphism of \tref{scheme}{schemes}, and assume that all fibers $X_y$ are \tref{noetherian-scheme}{locally noetherian}. Then $f$ is \textbf{regular} at $x \in X$ if $f$ is \tref{flat-morphism}{flat} at $x$ and the fiber $X_{f(x)}$ is \tref{geometrically-regular}{geometrically regular} at $x$ over $k(y)$. The morphism $f$ is \textbf{regular} if it is regular at all points $x \in X$.
\end{topic}
...
\begin{example}{e-polynomial}
    ...
    which shows that
    \[ e(X) = e(Z) + e(U) . \]
    Moreover, it can be shown that $e(X \times_\CC Y) = e(X) e(Y)$. This shows that $e \colon \textbf{Var}_\CC \to \ZZ[u, v]$ factors through the \tref{grothendieck-ring-of-varieties}{Grothendieck ring of varieties}.
\end{example}
...
\begin{topic}{zariski-main-theorem}{Zariski's main theorem}
    \textbf{Zariski's main theorem} states that any \tref{separated-morphism}{separated} \tref{quasi-finite-morphism}{quasi-finite} morphism $f \colon Y \to X$ with $X$ \tref{quasi-compact-scheme}{quasi-compact}, factors as $f' \circ g$, where $f' \colon Y \to Y'$ is an \tref{open-immersion}{open immersion} and $g \colon Y' \to X$ is a \tref{finite-morphism}{finite morphism}.
\end{topic}
...
\begin{topic}{rationally-connected-variety}{rationally connected variety}
    A \textbf{rationally connected variety} is a \tref{projective-variety}{projective variety} $X$ over an \tref{AA:algebraically-closed-field}{algebraically closed field} $k$ such that through every two points passes the image of a morphism $\gamma \colon \PP^1_k \to X$.
\end{topic}
...
\begin{topic}{kapranov-zeta-function}{Kapranov zeta function}
    ...
    For any ring morphism $\mu \colon \textup{K}_0(\textbf{Var}_k) \to R$, the \textbf{motivic zeta function} of $X$ is given by
...
\begin{example}{kapranov-zeta-function}
    ...
    \[ Z(-, t) \colon \textup{K}_0(\textbf{Var}_k) \to 1 + t \cdot \textup{K}_0(\textbf{Var}_k) \llbracket t \rrbracket , \]
...
\begin{topic}{luna-slice-theorem}{Luna slice theorem}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \[ \psi \colon G \times_{G_x} Y \to X, \quad (g, y) \mapsto g \cdot y , \]
...
\begin{topic}{moduli-space}{moduli space}
    Let $F \colon \textbf{Sch}^\textup{op} \to \textbf{Set}$ be a \tref{CT:functor}{functor}.
...
\begin{topic}{valuative-criterion}{valuative criterion}
    Let $f \colon X \to Y$ be a morphism of \tref{scheme}{schemes} over a scheme $S$, and assume $f$ is \tref{finite-type}{of finite type} and \tref{separated-morphism}{quasi-separated}. The \textbf{valuative criterion} states that $f$ is \tref{proper-morphism}{proper} if and only if for every diagram
    \[ \svg \begin{tikzcd}
...
\begin{example}{valuative-criterion}
    The valuative criterion for properness can be used to show that the morphism $f \colon \AA^1_k \to \Spec k$ is not proper. Namely, take $R = k[t]_{(t)}$ with $K = k(t)$. Considering the diagram with $\AA^1_k \to \Spec K$ given by $k[x] \to k(t)$, $x \mapsto 1/t$, there does not exist a map $k[x] \to k[t]_{(t)}$ completing the diagram.
\end{example}

\begin{example}{valuative-criterion}
    The valuative criterion for properness can be used to show that the morphism $f \colon \PP^n \to \Spec \ZZ$ is proper. It can be assumed that $R$ is a discrete valuation ring. A morphism $\Spec K \to \PP^n$ can be expressed in terms of homogeneous coordinates $(a_0 : a_1 : \ldots : a_n)$, with $a_i \in K$ not all zero. Let $a_\textup{min} = \min \{ a_i : 1 \le i \le n, a_i \ne 0 \}$ with respect to the valuation of $R$. Then the $R$-point of $\PP^n$ we are looking for is given by
...
\begin{topic}{relative-spectrum}{relative spectrum}
    Let $S$ be a \tref{scheme}{scheme} and $\mathcal{A}$ a \tref{coherent-sheaf}{quasi-coherent} \tref{O-algebra}{sheaf of $\mathcal{O}_S$-algebras}. For any cover of $S$ by affine opens $\{ U_i \}$, the natural maps $\mathcal{O}_S(U_i) \to \mathcal{A}(U_i)$ induce morphisms $\pi_i \colon \Spec(\mathcal{A}(U_i)) \to U_i$, which glue together to a scheme
    \[ \pi \colon \underline{\Spec}_S(\mathcal{A}) \to S , \]
...
\begin{topic}{blowup}{blowup}
    ...
    \[ \pi \colon \textup{Bl}_Z(X) \to X \]
    with the universal property that for every morphism $f \colon Y \to X$ such that $f^* \mathcal{I}$ is \tref{invertible-sheaf}{invertible}, there exists a unique morphism $g \colon Y \to \textup{Bl}_Z(X)$ such that $\pi \circ g = f$.
...
\begin{example}{blowup}
    ...
    \[ f \colon (X \setminus Z) \to \PP^{r - 1}, \quad x \mapsto (f_1(x) : \cdots : f_r(x)) , \]
    the blowup of $X$ at $Z$ is the closure of the graph $\Gamma_f \subset X \times \PP^{r - 1}$,
    \[ \pi \colon \textup{Bl}_Z(X) := \overline{\Gamma}_f \to X . \]
...
\begin{topic}{strict-transform}{strict transform}
    Let $X$ be a \tref{scheme}{scheme} with \tref{closed-immersion}{closed subscheme} $Z \subset X$ and \tref{blowup}{blowup} $\pi \colon \textup{Bl}_Z(X) \to X$. The \textbf{strict transform} of a closed subscheme $Y \subset X$ is the closure of $\pi^{-1}(Y \setminus Z)$ inside $\textup{Bl}_Z(X)$.
\end{topic}
...
\begin{topic}{chow-lemma}{Chow's lemma}
    % Let $S$ be a \tref{noetherian-scheme}{noetherian} \tref{scheme}{scheme}, and $X$ a scheme \tref{proper-morphism}{proper} over $S$. Then \textbf{Chow's lemma} states that there exists a scheme $X'$ which is \tref{projective-morphism}{projective} over $S$, and a surjective morphism $f \colon X' \to X$ over $S$, which induces an isomorphism $f^{-1}(U) \isom U$ for some \tref{TO:dense}{dense} open $U \subset X$.
    Let $S$ be a \tref{noetherian-scheme}{noetherian} \tref{scheme}{scheme}, and $f \colon X \to S$ a \tref{separated-morphism}{separated morphism} of \tref{finite-type}{finite type}. Then \textbf{Chow's lemma} states that there exists a commutative diagram
    \[ \svg \begin{tikzcd}
        X \arrow[swap]{dr}{f} & X' \arrow[swap]{l}{\pi} \arrow{r}{i} \arrow{d} & \PP^n_S \arrow{dl} \\ & S &
    \end{tikzcd} \]
    with $\pi \colon X' \to X$ \tref{proper-morphism}{proper} and surjective, $i \colon X' \to \PP^n_S$ an \tref{immersion}{immersion}, such that $\pi$ restricts to an isomorphism $\pi^{-1}(U) \isom U$ for some \tref{TO:dense-set}{dense} open $U \subset X$.
 \end{topic}

\begin{topic}{normal-crossings-divisor}{normal crossings divisor}
    Let $X$ be a \tref{scheme}{scheme} over a base scheme $S$. A \textbf{normal crossings divisor} of $X$ is a \tref{closed-immersion}{closed subscheme} $D$ of $X$ such that for all $x \in X$, there exists an integer $n \ge 0$, a scheme $U/S$, an \tref{etale-morphism}{étale morphism} $f \colon U \to X$ with $x \in f(U)$, and an étale morphism $g \colon U \to \AA^n_S$, such that $f^* D = g^* Z$, where $Z = V(x_1 x_2 \cdots x_n) \subset \AA^n_S$ is the union of the coordinate hyperplanes.
\end{topic}

\begin{example}{normal-crossings-divisor}
    Let $k$ be a field, $X = \AA^2_k$, and $D = \Spec k[x, y] / (y^2 - x^3 - x)$. Then $D$ is a normal crossings divisor of $X$. In particular, at the point given by the maximal ideal $(x, y)$, take $U = \Spec k[x, y, t, s] / (t^2 - (x + 1), s (x + 1) - 1)$ with $f \colon U \to X$ the obvious morphism and $g \colon U \to \AA^2_k = \Spec k[u, v]$ the morphism given by $u \mapsto y - xt$ and $v \mapsto y + xt$. Then
    \[ \begin{aligned}
        f^* D
...
\begin{topic}{analytification}{analytification}
    ...
    The set of complex points $X(\CC)$ naturally has a \tref{TO:topological-space}{topology}, such that for any affine open $U \subset X$ the topology on $U(\CC) \subset \CC^n$ is given by the \tref{TO:subspace-topology}{subspace topology} of the Euclidean topology on $\CC^n$. These charts give $X(\CC)$ the structure of a \tref{DG:complex-manifold}{complex manifold} $X^\textup{an}$, called the \textbf{analytification} of $X$. Moreover, a morphism of non-singular complex varieties $f \colon X \to Y$ induces a holomorphic map $f^\textup{an} \colon X^\textup{an} \to Y^\textup{an}$, and this construction yields the \textit{analytification functor}
    \[ (-)^\textup{an} \colon \textbf{SmVar}_\CC \to \textbf{Mfld}_\CC . \]
\end{topic}

\begin{topic}{etale-fundamental-group}{étale fundamental group}
    Let $X$ be a \tref{scheme}{scheme} over a field $k$, and let $\overline{x} \to X$ be a \tref{geometric-point}{geometric point} of $X$. Let $\textbf{FÉt}/X$ be the category of \tref{finite-morphism}{finite} \tref{etale-morphism}{étale} coverings $\pi \colon Y \to X$ over $X$. The functor
    \[ F \colon \textbf{FÉt}/X \to \textbf{Set}, \quad Y \mapsto \Hom_X(\overline{x}, Y) \]
...
\begin{example}{etale-fundamental-group}
    ...
    \[ \pi_n \colon X_n = \AA^1_k \setminus \{ 0 \} \to X, \quad x \mapsto x^n , \]
...
\begin{topic}{larsen-lunts-theorem}{Larsen--Lunts theorem}
    ...
    \[ \phi \colon \textup{K}_0(\textbf{Var}_\CC) \to \ZZ[\textup{SB}] , \]
...
\begin{topic}{riemann-hurwitz-formula}{Riemann--Hurwitz formula}
    Let $f \colon C' \to C$ be a morphism between \tref{non-singular-variety}{smooth} \tref{projective-variety}{projective} \tref{algebraic-curve}{curves} $C'$ and $C$. The \textbf{Riemann--Hurwitz formula} states that
...
%     \end{array} \]
%     and \textit{truncation maps} $\pi^k_m \colon X_k \to X_m$ for $k \ge m$ and $\psi_m \colon X_\infty \to X_m$, induced by the natural maps $\Delta_m \to \Delta_k$ and $\Delta_\infty \to \Delta_m$, respectively.
...
% \[ F_D \colon J_\infty(X) \to \ZZ_{\ge 0} \cup \{ \infty \} \]
...
% For a complex variety $X$ with at worst Gorenstein canonical singularities, the \textbf{motivic integral} of $X$ is the motivic integral of $(Y, D)$, where $\varphi \colon Y \to X$ is any \tref{resolution-of-singularities}{resolution of singularities} for which $D = K_Y - \varphi^* K_X$ has only simple normal crossings.
...
\begin{topic}{motivic-integration}{motivic integration}
    ...
    \[ \pi^m_n \colon J_m(X) \to J_n(X) \quad \textup{ and } \quad \pi_m \colon J_\infty(X) \to J_m(X) . \]
    ...
    For any cylinder $A \subset J_\infty(X)$ and function $\alpha \colon A \to \ZZ \cup \{ \infty \}$ such that $\alpha^{-1}(k) \subset J_\infty(X)$ are also cylinders for all $k \ge 0$, the \textbf{motivic integral} of $\mathbb{L}^{-\alpha}$ over $A$ is defined as
...
\begin{example}{motivic-integration}
    Let $X = \AA^1_\CC$ and consider $\alpha \colon J_\infty(X) \to \ZZ \cup \{ \infty \}$ given by $\alpha(\gamma) = \operatorname{ord}_t(\gamma)$. Then
...
\begin{example}{motivic-integration}
    Let $X \subset \AA^2_\CC$ be the closed subvariety given by the equation $xy = 0$. The formal arc space of $X$ is given by
    \[ J_\infty(X) = \{ (\gamma_1, \gamma_2) \in \CC \llbracket t \rrbracket^2 \mid \gamma_1 \gamma_2 = 0 \} . \]
    Note that $\gamma_1 \gamma_2 = 0$ implies $\gamma_1 = 0$ or $\gamma_2 = 0$. Therefore, the image of $J_\infty(X)$ under $\pi_m$ is not equal to $J_m(X)$ but rather is given by
    \[ \pi_m(J_\infty(X)) = \{ (\gamma_1, \gamma_2) \in (\CC[t] / (t^{m + 1})^2 \mid \gamma_1 = 0 \textup{ or } \gamma_2 = 0 \} , \]
    whose virtual class is $[\pi_m(J_\infty(X))] = 2 \mathbb{L}^{m + 1} - 1$. It follows, for $\alpha = 0$, that
    \[ \int_{J_\infty(X)} \mathbb{L}^{-\alpha} d \mu = \mu(J_\infty(X)) = \lim_{m \to \infty} [\pi_m(J_\infty(X))] / \mathbb{L}^{m + 1} = 2 . \]
\end{example}

\begin{topic}{nisnevich-morphism}{Nisnevich morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{Nisnevich} if it is \tref{etale-morphism}{étale} and for every point $y \in Y$, there exists a point $x \in X$ with $f(x) = y$ such that the induced map on residue fields $k(y) \to k(x)$ is an isomorphism.
\end{topic}

\begin{example}{nisnevich-morphism}
    The morphism
    \[ f \colon \Spec \CC[x^{\pm 1}, y] / (y^2 - x) \to \Spec \CC[x^{\pm 1}] \]
    is étale, but not Nisnevich, because the induced map on residue fields for the generic point
    \[ f^\# \colon \CC(x) \to \CC(x)[y] / (y^2 - x) \]
    is not an isomorphism. However, if we add $\Spec \CC(x) \to \Spec \CC[x^{\pm 1}]$ to the morphism, then
    \[ \tilde{f} \colon \Spec \CC[x^{\pm 1}, y] / (y^2 - x) \sqcup \Spec \CC(x) \to \Spec \CC[x^{\pm 1}] , \]
    is Nisnevich.
\end{example}
...
\begin{topic}{semi-separated-morphism}{semi-separated morphism}
    A morphism $f \colon X \to Y$ of \tref{scheme}{schemes} is \textbf{semi-separated} if the diagonal $\Delta_{X/Y} \colon X \to X \times_Y X$ is \tref{affine-morphism}{affine}.
\end{topic}

\begin{example}{semi-separated-morphism}
    If $f \colon X \to S$ is a semi-separated morphism with $S$ affine, then intersections of open affines on $X$ are again open affine. That is, for two open affines $U, V \subset X$, their intersection is given by
...
\begin{topic}{weak-factorization-theorem}{weak factorization theorem}
    Let $\phi \colon X \dashrightarrow Y$ be a \tref{birational-map}{birational map} between \tref{complete-variety}{complete} \tref{non-singular-variety}{non-singular} \tref{TO:connected-space}{connected} \tref{variety}{varieties} over a \tref{AA:field}{field} $k$ of characteristic zero, and let $U \subset X$ be an open subset where $\phi$ is an isomorphism.
    ...
    such that each $\phi_i$ is an isomorphism over $U$ and $\phi_i \colon V_{i - 1} \dashrightarrow V_i$ or $\phi_i^{-1} \colon V_i \dashrightarrow V_{i - 1}$ is obtained by \tref{blowup}{blowing up} a smooth center disjoint from $U$.
...
\begin{topic}{grothendieck-verdier-duality}{Grothendieck--Verdier duality}
    Let $f \colon X \to Y$ be a morphism between \tref{smooth-morphism}{smooth} \tref{scheme}{schemes} over a \tref{AA:field}{field} $k$. For any $\mathcal{F} \in \textbf{D}^\textup{b}(Y)$, define
...
\begin{topic}{crepant-resolution}{crepant resolution}
    Let $X$ be a \tref{variety}{variety} over a field $k$. A \tref{resolution-of-singularities}{resolution} $\pi \colon \widetilde{X} \to X$ is \textbf{crepant} if the canonical isomorphism $\pi^* K_U \cong K_{\pi^{-1}(U)}$ extends to an isomorphism $\pi^* K_X \cong K_{\widetilde{X}}$, where $U \subset X$ is a non-singular open subset such that $\pi \colon \pi^{-1}(U) \to U$ is an isomorphism.
\end{topic}
...
\begin{topic}{riemann-existence-theorem}{Riemann existence theorem}
    ...
    \[ \textbf{FÉt}/X \simeq \textbf{FCov}(X(\CC)), \quad (\pi \colon Y \to X) \mapsto (\pi \colon Y(\CC) \to X(\CC)) \]
...
\begin{topic}{nagata-compactification-theorem}{Nagata's compactification theorem}
    Let $f \colon X \to S$ be a \tref{separated-morphism}{separated} morphism of \tref{finite-type}{finite type} with $S$ a \tref{noetherian-scheme}{noetherian scheme}. \textbf{Nagata's compactification theorem} states that $f$ factors as an \tref{open-immersion}{open immersion} $j \colon X \to \overline{X}$ followed by a \tref{proper-morphism}{proper morphism} $p \colon \overline{X} \to S$.
\end{topic}
