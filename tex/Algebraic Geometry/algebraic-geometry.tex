...
\begin{topic}{integral-morphism}{integral morphism}
    A morphism $f : X \to Y$ of \tref{scheme}{schemes} is \textbf{integral} if it is \tref{affine-morphism}{affine} and for every affine open $\Spec B = V \subseteq Y$ with inverse image $\Spec A = f^{-1}(V) \subseteq X$ the corresponding ring morphism $B \to A$ is \tref{AA:integral-closure}{integral}.
\end{topic}

\begin{topic}{radical-morphism}{radical morphism}
    A morphism $f : X \to Y$ of \tref{scheme}{schemes} is \textbf{radical} if it is injective (on the level of topological spaces) and for every $x \in X$ the \tref{AA:field-extension}{field extension} $\kappa(x) / \kappa(f(x))$ is \tref{AA:purely-inseparable-field-extension}{purely inseparable}.
\end{topic}
