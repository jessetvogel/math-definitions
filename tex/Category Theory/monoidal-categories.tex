\begin{topic}{monoidal-category}{monoidal category}
    A \textbf{monoidal category} is a \tref{category}{category} $\mathcal{C}$ with a functor $\otimes \colon \mathcal{C} \times \mathcal{C} \to \mathcal{C}$ (the \textit{tensor product}), an object $\textbf{1}$ in $\mathcal{C}$ (the \textit{unital object}) and \tref{natural-transformation}{natural} isomorphisms
    \[ \begin{aligned}
...
\begin{topic}{braided-monoidal-category}{braided monoidal category}
    ...
    % \[ \gamma_{X, Y} \colon X \otimes Y \to Y \otimes X \]
...
\begin{topic}{monoidal-functor}{(lax) monoidal functor}
    A \textbf{lax monoidal functor} is a \tref{functor}{functor} $F \colon \mathcal{C} \to \mathcal{D}$ between \tref{monoidal-category}{monoidal categories} together with a \tref{natural-transformation}{natural transformation}
...
\begin{example}{dualizable-object}
    Let $R$ be a \tref{AA:ring}{commutative ring}. An object in $(\cat{Mod}_R, \otimes_R, R)$ is dualizable if and only if it is a \tref{AA:finitely-generated-module}{finitely generated} \tref{AA:projective-module}{projective} $R$-module.
\end{example}
...
\begin{topic}{ribbon-category}{ribbon category}
    A \textbf{ribbon category} is a \tref{rigid-monoidal-category}{rigid} \tref{braided-monoidal-category}{braided monoidal category} $(\mathcal{C}, \otimes, \textbf{1}, \tau)$ together with a collection of isomorphisms $\theta_X \colon X \to X$, called a \textit{twist}, satisfying
    \begin{itemize}
...
\begin{example}{ribbon-category}
    ...
    \begin{itemize}
        ...
        \item The dual of an $A$-module $V$ is the \tref{LA:dual-vector-space}{dual vector space} $V^*$ with $(a \cdot \varphi)(v) = \varphi(S^{-1}(a) \cdot v)$. There are morphisms $\eta_V \colon k \to V \otimes_k V^*$ and $\varepsilon_V \colon V^* \otimes_k V \to k$ given by $\eta_V(1) = \sum_i v_i \otimes v^i$ and $\varepsilon(\varphi \otimes v) = \varphi(v)$. 
...
\begin{topic}{module-category}{module category}
    Let $(\mathcal{C}, \otimes, \textbf{1})$ be a \tref{monoidal-category}{monoidal category}. A \textbf{(left) module category} over $\mathcal{C}$ is a category $\mathcal{M}$ together with a \tref{functor}{functor} $\boxtimes \colon \mathcal{C} \times \mathcal{M} \to \mathcal{M}$ and \tref{natural-transformation}{natural} isomorphisms
    \[ \begin{aligned}
...
\begin{topic}{fusion-category}{fusion category}
    A \textbf{fusion category} is a \tref{monoidal-category}{monoidal category} which is \tref{rigid-monoidal-category}{rigid}, \tref{HA:linear-category}{$k$-linear} for some \tref{AA:field}{field} $k$, \tref{HA:semisimple-category}{semisimple}, and which has only finitely many \tref{HA:simple-object}{simple objects} up to isomorphism.
\end{topic}

\begin{topic}{category-center}{category center}
    The \textbf{center} of a \tref{monoidal-category}{monoidal category} $(\mathcal{C}, \otimes, \textbf{1})$ is the category $Z(\mathcal{C})$ defined as follows.
    \begin{itemize}
        \item Its objects are pairs $(X, \mu)$ with $X$ an object of $\mathcal{C}$ and $\mu \colon X \otimes (-) \Rightarrow (-) \otimes X$ a \tref{natural-transformation}{natural isomorphism}, such that $\mu_{Y \otimes Z} = (\id_Y \otimes \mu_Z) \circ (\mu_Y \otimes \id_Z)$ for all objects $Y$ and $Z$ of $\mathcal{C}$.
