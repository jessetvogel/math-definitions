...
\begin{topic}{simplicial-object}{simplicial object}
    ...
    where $\Delta$ denotes the \tref{simplex-category}{simplex category}. Concretely, a simplicial object consists of a family of objects $X_n$ in $\mathcal{C}$ and a morphism $X_m \to X_n$ for each function $f \colon [n] \to [m]$, which compose nicely.
...
\begin{example}{nerve}
    Consider a \tref{GT:group}{group} $G$ as a category with one object. Note that any $f \in \textup{N}_n(G)$ can be described by the images $f(k - 1 \le k) \in G$ for $1 \le k \le n$, so that $\textup{N}_n(G) \isom G^n$. Hence, the nerve $\textup{N}_\bdot(G)$ is the simplicial set
