...
\begin{example}{spectral-sequence}
    Let $C^{\bdot, \bdot}$ be a \tref{double-complex}{double complex} with total complex $C^\bdot$, over an abelian category $\mathcal{A}$. It is a theorem that there exists the \textit{double complex spectral sequence}
    \[ E_0^{p, q} = C^{p, q} \Rightarrow H^{p + q}(C^\bdot) , \]
    and by symmetry also $E_0^{p, q} = C^{q, p} \Rightarrow H^{p + q}(C^\bdot)$. These spectral sequences can be used to prove the \tref{snake-lemma}{Snake lemma}. Consider the diagram
    \[ \begin{tikzcd}
    ...
    \[ \begin{array}{c}
        ...
        E_2^{p, q} = \quad \begin{tikzcd}[row sep=0.5em, column sep=0.5em]
            ...
            0 & 0 & 0 & \ker c / \ker b
        \end{tikzcd} \begin{tikzpicture}[overlay,remember picture]
            \draw[-latex] (-7.0, -0.6) -- (-7.0, 0.65);
            \draw[-latex] (-7.0, -0.6) -- (0, -0.6);
            \node at (0.2, -0.6) {$p$};
            \node at (-7.25, 0.7) {$q$};
        \end{tikzpicture}
    \end{array} \]
...
\begin{example}{semisimplification}
    ...
    \[ V^\textup{ss} = V_1/V_0 \oplus V_2/V_1 \simeq k^2 , \]
...
\begin{example}{homological-dimension}
    By \tref{AA:hilbert-syzygy-theorem}{Hilbert's syzygy theorem}, any $k[x_1, \ldots, x_n]$-module has a projective resolution of length $\le n$. Since this upper bound is attained for the \tref{AA:koszul-complex}{Koszul complex}, the global homological dimension of $k[x_1, \ldots, x_n]\text{-}\textbf{Mod}$ is $n$.
\end{example}
