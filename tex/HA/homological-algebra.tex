...
\begin{topic}{abelian-category}{abelian category}
    ...
    \begin{itemize}
        \item every morphism $f$ has a kernel (that is, an \tref{CT:equalizer}{equalizer} of $f$ and $0$) and a cokernel (that is, a \tref{CT:coequalizer}{coequalizer} of $f$ and $0$),
...
\begin{topic}{right-derived-functors}{right derived functors}
    ...
    \[ \textup{R}^i F M = H^i(F(I^\bdot)) . \]
    This is well-defined (independent of the chosen resolution) and yields additive functors $\textup{R}^i F : \mathcal{A} \to \mathcal{B}$, called the \textbf{right derived functors} of $F$. Note that $R^0 F \isom F$.
    
    For each short exact sequence $0 \to A \to B \to C \to 0$ in $\mathcal{A}$ there is an associated long exact sequence
    \[ 0 \to FA \to FB \to FC \to \textup{R}^1FA \to \textup{R}^1FB \to \textup{R}^1FC \to \textup{R}^2FA \to \cdots \]
\end{topic}
...
\begin{topic}{left-derived-functors}{left derived functors}
    ...
    \[ \textup{L}_i F M = H^i(F(P^\bdot)) . \]
    This is well-defined (independent of the chosen resolution) and yields additive functors $\textup{L}_i F : \mathcal{A} \to \mathcal{B}$, called the \textbf{left derived functors} of $F$. Note that $\textup{L}_0 F \isom F$.
    
    For each short exact sequence $0 \to A \to B \to C \to 0$ in $\mathcal{A}$ there is an associated long exact sequence
    \[ \cdots \to \textup{L}_2FC \to \textup{L}_1FA \to \textup{L}_1FB \to \textup{L}_1FC \to FA \to FB \to FC \to 0 \]
\end{topic}
...
\begin{topic}{ext-functors}{Ext functors}
    ...
    \[ \textup{Ext}^i_\mathcal{A}(A, B) = \textup{R}^i \Hom_\mathcal{A}(A, B) . \]
\end{topic}
...
\begin{topic}{tor-functors}{Tor functors}
    ...
    \[ \textup{Tor}^R_i(A, B) = A \overset{\textup{L}_i}{\otimes} B . \]
\end{topic}
...
\begin{example}{homological-dimension}
    Knowing the global homological dimension $n$ of a category $\mathcal{A}$ is useful, as it follows that for any \tref{exact-functor}{right exact functor} $F : \mathcal{A} \to \mathcal{B}$ the \tref{left-derived-functors}{left derived functors} $\textup{L}^i F A$ are zero for all $A$ in $\mathcal{A}$ and $i > n$.
...
\begin{example}{group-cohomology}
    ...
    \[ H^n(G, M) \isom \textup{R}^i \Hom_{\ZZ[G]}(\ZZ, M) = \textup{Ext}^n_{\ZZ[G]}(\ZZ, M) . \]
\end{example}
