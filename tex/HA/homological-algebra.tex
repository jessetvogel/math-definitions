...
\begin{example}{spectral-sequence}
    ...
    with differential $d_0 : E_0^{p, q} \to E_0^{p, q + 1}$ given by the differential $d$ of $C^\bdot$. On page $r \ge 1$, one puts
    \[ \begin{aligned}
        Z_r^{p, q} &= \ker (d : F^p C^{p + q} \to C^{p + q + 1} / F^{p + r} C^{p + q + 1}) \\
        B_r^{p, q} &= F^{p + 1} C^{p + q} + d(F^{p - r + 1} C^{p + q - 1}) \\
        E_r^{p, q} &= Z_r^{p, q} / (B_r^{p, q} \cap Z_r^{p, q})
    \end{aligned} \]
    with differential $d_r$ induced by the differential $d : Z_r^{p, q} \to Z_r^{p + r, q - r + 1}$. On page $r = \infty$, one puts
    \[ \begin{aligned}
        Z_\infty^{p, q} &= \ker(d : F^p C^{p + q} \to K^{p + q + 1}) \\
...
\begin{topic}{hypercohomology}{hypercohomology}
    Let $F : \mathcal{A} \to \mathcal{B}$ be an \tref{exact-functor}{exact functor} between \tref{abelian-category}{abelian categories}, and suppose $\mathcal{A}$ has \tref{enough-injectives}{enough injectives}. For any \tref{chain-complex}{cochain complex} $A^\bdot$ in $\mathcal{A}$, the \textbf{hypercohomology} of $A^\bdot$, denoted
    \[ \mathbb{H}^i(A^\bdot) \]
