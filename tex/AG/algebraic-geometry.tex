...
\begin{topic}{chow-motive}{Chow motive}
    Let $\mathcal{V}(k)$ be the \tref{CT:category}{category} whose objects are \tref{smooth-morphism}{smooth} \tref{projective-morphism}{projective} schemes over a field $k$, and whose morphisms are given by the \tref{chow-group}{Chow groups}
    \[ \Hom_{\mathcal{V}(k)}(X, Y) = \textup{CH}_{\dim X}(X \times_k Y) . \]
    Composition is given by
    \[ \beta \circ \alpha = \pi_{XZ*}(\pi_{XY}^*(\alpha) \cdot \pi_{YZ}^*(\beta)) \]
    for all $\alpha \in \Hom_{\mathcal{V}(k)}(X, Y)$ and $\beta \in \Hom_{\mathcal{V}(k)}(Y, Z)$, and for any $X$ the identity $\id_X$ is given by the diagonal $\Delta_X(X) \in \textup{CH}_{\dim X}(X, X)$.
...
\begin{example}{kapranov-zeta-function}
    Let $\mathbb{L} = [\AA^1] \in \textup{K}(\textbf{Var}/k)$ be the class of the affine line. By the \tref{AA:symmetric-polynomial}{fundamental theorem of symmetric polynomials}, we have $\operatorname{Sym}^n(\AA^1) \simeq \AA^n$ for all $n \ge 0$. More generally, one can show
...
\begin{topic}{chow-lemma}{Chow's lemma}
    ...
    \end{tikzcd} \]
    with $\pi : X' \to X$ \tref{proper-morphism}{proper} and surjective, $i : X' \to \PP^n_S$ an \tref{immersion}{immersion}, such that $\pi$ restricts to an isomorphism $\pi^{-1}(U) \simeq U$ for some \tref{TO:dense-set}{dense} open $U \subset X$.
 \end{topic}
...
\begin{topic}{motivic-integration}{motivic integration}
    Let $X$ be a \tref{nonsingular-variety}{smooth} complex \tref{variety}{variety} of dimension $n$. Define
    \[ \begin{array}{cc}
