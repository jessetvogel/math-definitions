...
\begin{topic}{quotient-stack}{quotient stack}
    If $G$ is an \tref{algebraic-group}{algebraic group} acting on a \tref{scheme}{scheme} $S$, the \textbf{quotient stack} $[S/G]$ is defined as the \tref{category}{category} over $\textbf{Sch}$ whose objects over $T$ are principal $G$-bundles $P \to T$, with a $G$-equivariant morphism $P \to S$. Its morphisms are $G$-equivariant isomorphisms which are compatible with the morphisms to $S$.
\end{topic}
