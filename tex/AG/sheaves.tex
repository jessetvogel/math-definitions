...
\begin{topic}{constant-sheaf}{constant sheaf}
    Let $X$ be a \tref{TO:topological-space}{topological space}, and $A$ an \tref{GT:abelian-group}{abelian group}. The \textbf{constant sheaf} $\underline{A}$ on $X$ determined by $A$ is the sheaf given by
...
\begin{topic}{direct-image-sheaf}{direct image sheaf}
    ...
    which is \tref{CT:adjoint-functors}{right adjoint} to the \tref{inverse-image-sheaf}{inverse image functor} $f^{-1}$.
...
\begin{topic}{inverse-image-sheaf}{inverse image sheaf}
    ...
    It is the \tref{CT:adjoint-functors}{left adjoint} of the \tref{direct-image-sheaf}{direct image functor} $f_* : \textup{Sh}(X) \to \textup{Sh}(Y)$.
...
\begin{topic}{coherent-sheaf}{(quasi-)coherent sheaf}
    ...
    Furthermore, $\mathcal{F}$ is \textbf{coherent} is each $M_i$ can be taken to be a \tref{CA:finitely-generated-module}{finitely generated} $R_i$-module.
\end{topic}
...
\begin{topic}{sheaf-rational-functions}{sheaf of rational functions}
    Let $X$ be a \tref{scheme}{scheme}. The \textbf{sheaf of rational functions} $K_X$ on $X$ is the \tref{associated-sheaf}{sheafification} of the presheaf which assigns to an open $U \subset X$ the \tref{CA:localization}{localization} $S^{-1} \mathcal{O}_X(U)$, where $S$ is the multiplicative set of all $f \in \mathcal{O}_X(U)$ which are stalk-wise not zero-divisors.
\end{topic}
...
\begin{example}{sheaf-cohomology}
    Let $X = \Spec A$ for a \tref{CA:noetherian-ring}{noetherian ring} $A$, and $\mathcal{F}$ a \tref{coherent-sheaf}{quasi-coherent sheaf}. Then $H^i(X, \mathcal{F}) = 0$ for $i > 0$. Namely, write $M = \Gamma(X, \mathcal{F})$ and take an injective resolution
    \[ 0 \to M \to I^\bdot \]
