...
\begin{topic}{extension-by-zero}{extension by zero}
    Let $X$ be a \tref{TO:topological-space}{topological space}, $i : Z \to X$ the inclusion of a closed subset, and $j : U \to X$ the inclusion of the complement $U = X \setminus Z$. For any sheaf of abelian groups $\mathcal{F}$ on $U$, the \textbf{extension by zero} of $\mathcal{F}$ is the sheaf $j_! \mathcal{F}$ on $X$ \tref{associated-sheaf}{associated} to the presheaf
...
\begin{topic}{direct-image-sheaf-compact-support}{direct image sheaf with compact support}
    Let $f : X \to Y$ be a map of \tref{TO:topological-space}{topological spaces}, and let $\mathcal{F}$ be a \tref{sheaf}{sheaf} on $X$. The \textbf{direct image sheaf with compact support} $f_! \mathcal{F}$ on $Y$ is defined by
    \[ (f_! \mathcal{F})(V) = \{ s \in \mathcal{F}(f^{-1}(V)) \mid f_{\operatorname{supp}(s)} : \operatorname{supp}(s) \to V \textup{ is proper} \} \]
    for any open set $V \subset Y$ (indeed this presheaf is a sheaf).
\end{topic}
...
\begin{topic}{perverse-sheaf}{perverse sheaf}
    ...
    \end{aligned} \]
    where $K^\vee = R\iHom(K, \pi^! F)$ denotes the dual of $K$, with $\pi : X \to \Spec k$. The heart of this $t$-structure
