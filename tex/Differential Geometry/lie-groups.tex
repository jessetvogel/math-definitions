\begin{topic}{lie-group}{Lie group}
    A \textbf{Lie group} is a \tref{GT:group}{group} $G$ which is also a finite-dimensional \tref{smooth-manifold}{smooth manifold}, such that the multiplication map $G \times G \to G \colon (x, y) \mapsto xy$ and the inversion map $G \to G : x \mapsto x^{-1}$ are \tref{smooth-map}{smooth}.
\end{topic}
...
\begin{topic}{adjoint-representation}{(co)adjoint representation}
    ...
    where $c_g \colon G \to G, h \mapsto g h g^{-1}$ denotes the \tref{GT:conjugation}{conjugation} map.
    
    The \textbf{co-adjoint representation} $\textup{Ad}_G^* \colon G \to \textup{GL}(\mathfrak{g}^*)$ is the \tref{RT:dual-representation}{dual representation} of $\textup{Ad}_G$.
...
\begin{topic}{coadjoint-orbit}{coadjoint orbit}
    Let $G$ be a \tref{lie-group}{Lie group} and $\mathfrak{g}$ its corresponding \tref{AA:lie-algebra}{Lie algebra}. The \textbf{coadjoint orbit} of some $\xi \in \mathfrak{g}^*$ is the \tref{GT:orbit}{orbit} of the \tref{adjoint-representation}{coadjoint representation} $\textup{Ad}^*_G \colon G \to \textup{GL}(\mathfrak{g}^*)$, that is
...
\begin{topic}{lie-theorems}{Lie's theorems}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item If $G$ and $H$ are Lie groups with Lie algebras $\mathfrak{g}$ and $\mathfrak{h}$, and $G$ is \tref{TO:simply-connected-space}{simply connected}, then any morphism of Lie algebras $f \colon \mathfrak{g} \to \mathfrak{h}$ lifts uniquely to a morphism of Lie groups $F \colon G \to H$, that is, $dF_e = f$.
...
\begin{topic}{poisson-lie-group}{Poisson--Lie group}
    A \textbf{Poisson--Lie group} is a \tref{lie-group}{Lie group} with a \tref{poisson-manifold}{Poisson structure} $\{ \cdot, \cdot \}$, such that multiplication $\mu \colon G \times G \to G$ is a \tref{poisson-map}{Poisson map}, where $G \times G$ is given the product Poisson structure. Explicitly, it is required that
...
\begin{topic}{lie-groupoid}{Lie groupoid}
    ...
    \begin{itemize}
        \item inversion $(-)^{-1} \colon X_1 \to X_1$, composition $\circ \colon X_1 \times_{X_0} X_1 \to X_1$ and the identity map $\id_{(-)} \colon X_0 \to X_1$, are all \tref{smooth-map}{smooth},
        \item the source and target maps $s, t \colon X_1 \to X_0$ are smooth surjective \tref{submersion}{submersions}.
    \end{itemize}
\end{topic}
