...
\begin{example}{norm}
    Every \tref{LA:inner-product}{inner product} $\langle \cdot, \cdot \rangle$ induces a norm $\norm{x} = \langle x, x \rangle$.
\end{example}
...
\begin{topic}{star-algebra}{*-algebra}
    A \textbf{*-algebra} is a \tref{banach-algebra}{Banach algebra} $A$ over $\CC$ together with a map $(-)^* \colon A \to A, x \mapsto x^*$ satisfying
    \begin{itemize}
...
\begin{example}{c-star-algebra}
    The algebra $\textup{Mat}_n(\CC)$ of $n \times n$ matrices over $\CC$ becomes a C*-algebra if we use \textit{operator norm}
...
\begin{example}{c-star-algebra}
    ...
    \[ C(X) = \{ f \colon X \to \CC \textup{ with $f$ continuous} \} \]
    with norm
...
\begin{topic}{gelfand-spectrum}{Gelfand spectrum}
    ...
    \[ \textup{sp}(A) = \{ \phi \colon A \to \CC \mid \phi \ne 0 \textup{ is an algebra morphism} \} , \]
    with the \textit{weak *-topology}: the \tref{TO:coarser}{coarsest} \tref{TO:topological-space}{topology} such that for all $a \in A$ the functions $\hat{a} \colon \textup{sp}(A) \to \CC$ given by $\hat{a}(\chi) = \chi(a)$ are continuous.
...
\begin{topic}{hilbert-space}{Hilbert space}
    A \textbf{Hilbert space} is an \tref{LA:inner-product}{inner product space} which is \tref{TO:complete-metric-space}{complete} with respect to the \tref{TO:metric-space}{metric} $d(x, y) = \langle x - y, x - y \rangle$ induced by the inner product.
\end{topic}

\begin{topic}{bounded-operator}{bounded operator}
    An operator $T \colon X \to Y$ between \tref{norm}{normed} \tref{LA:vector-space}{vector spaces} is \textbf{bounded} if there exists a number $K > 0$ such that $\norm{Tx} \le K \norm{x}$ for all $x \in X$.
\end{topic}

\begin{topic}{invertible-operator}{invertible operator}
    A \tref{bounded-operator}{bounded operator} $T \colon X \to Y$ between \tref{norm}{normed} \tref{LA:vector-space}{vector spaces} is \textbf{invertible} if there exists a bounded operator $S \colon Y \to X$ such that $ST = \id_X$ and $TS = \id_Y$.
\end{topic}

\begin{topic}{compact-operator}{compact operator}
    An operator $T \colon X \to Y$ between \tref{norm}{normed} \tref{LA:vector-space}{vector spaces} is \textbf{compact} if for every bounded subset $V \subset X$, the image $T(V)$ is relatively compact, that is, the closure $\overline{T(V)} \subset Y$ is compact.
\end{topic}
...
\begin{topic}{hilbert-schmidt-operator}{Hilbert--Schmidt operator}
    Let $H$ be a \tref{hilbert-space}{Hilbert space}. A \textbf{Hilbert--Schmidt operator} is an operator $A \colon H \to H$ with finite \textit{Hilbert--Schmidt norm}
...
\begin{example}{seminorm}
    ...
    \begin{itemize}
        ...
        \item For any linear map $f \colon V \to \RR$, the map $v \mapsto |f(v)|$ is a seminorm.
    \end{itemize}
\end{example}
...
\begin{topic}{schwartz-space}{Schwartz space}
    ...
    \[ \mathcal{S}(\RR^n) = \left\{ f \colon \RR^n \to \CC \mid \textup{for all } \alpha, \beta \in \NN^n , \sup_{x \in \RR^n} |x^\alpha \partial_x^\beta f| < \infty \right\} , \]
...
\begin{topic}{lp-space}{Lp space}
    ...
    \[ \mathcal{L}^p(X) = \left\{ f \colon X \to \mathbb{K} : \norm{f}_p < \infty \right\} , \]
    where
...
\begin{topic}{fredholm-operator}{Fredholm operator}
    Let $X$ and $Y$ be \tref{banach-space}{Banach spaces}. A \tref{bounded-operator}{bounded operator} $T \colon X \to Y$ is \textbf{Fredholm} if $\ker T$ and $\operatorname{coker} T$ are both finite-dimensional subspaces.
...
\begin{example}{fredholm-operator}
    ...
    has $\ker S = \{ 0 \}$ and $\coker S \isom \langle e_0 \rangle$, so $S$ is a Fredholm operator with index $-1$. The powers $S^k$ for $k \ge 0$ are also Fredholm with index $-k$.
...
\begin{example}{fredholm-operator}
    ...
    \[ S \colon (X / \ker T) \oplus C \to Y, \quad (x, c) \mapsto T(x) + c \]
...
\begin{topic}{open-mapping-theorem}{open mapping theorem}
    Let $X$ and $Y$ be \tref{banach-space}{Banach spaces} and $T \colon X \to Y$ a surjective continuous linear operator. The \textbf{open mapping theorem} states that $T$ is an open map. That is, for all open $U \subset X$, the image  $T(U) \subset Y$ is open as well.
\end{topic}
...
\begin{topic}{k-theory-c-star-algebra}{K-theory C*-algebra}
    ...
    \[ K_n(A) = K_0(S^n A) , \]
    where
    \[ SA = \{ f \colon [0, 1] \to A \mid f \textup{ continuous and } f(0) = f(1) = 0 \} \]
...
\begin{example}{k-theory-c-star-algebra}
    ...
    which shows that
    \[ \textup{K}_0(\CC) \isom \ZZ , \]
    where the isomorphism is given by the rank.
\end{example}

\begin{topic}{bott-periodicity}{Bott periodicity}
    Let $\textup{K}_n \colon \textbf{C*Alg} \to \textbf{Ab}$ denote the \tref{k-theory-c-star-algebra}{K-theory} of \tref{c-star-algebra}{C*-algebras}. \textbf{Bott periodicity} states that there are isomorphisms $\textup{K}_n \isom \textup{K}_{n + 2}$ for all $n$. In particular, for any short exact sequence of $C^*$-algebras $0 \to A \xrightarrow{i} B \xrightarrow{q} C \to 0$, the long exact sequence of $K$-theory collapses to a hexagon:
    \[ \svg \begin{tikzcd}
...
%     A \textbf{C*-category} is a \tref{CT:category}{category} $\mathcal{C}$ \tref{CT:enriched-category}{enriched} over the category of complex \tref{banach-space}{Banach spaces}, together with an antilinear involutive contravariant \tref{CT:functor}{functor} $(-)^* \colon \mathcal{C} \to \mathcal{C}$, such that
%     \begin{itemize}
