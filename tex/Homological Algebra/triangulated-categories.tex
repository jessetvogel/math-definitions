...
\begin{topic}{exact-functor-triangulated}{exact functor of triangulated categories}
    An \tref{additive-functor}{additive functor} between \tref{triangulated-category}{triangulated categories} $F \colon \mathcal{D} \to \mathcal{D}'$ is \textbf{exact} if
    \begin{itemize}
...
\begin{topic}{spanning-class}{spanning class}
    Let $\mathcal{T}$ be a \tref{triangulated-category}{triangulated category}. A collection $\Omega$ of objects in $\mathcal{T}$ is a \textbf{spanning class} (or spans $\mathcal{T}$) if for all $B \in \mathcal{T}$,
    \begin{itemize}
...
\begin{example}{verdier-quotient}
    ...
    where $\pi \colon \mathcal{T} \to \mathcal{S}$ denotes a \tref{CT:adjunction}{right adjoint} to the inclusion $i \colon \mathcal{S} \to \mathcal{T}$, and $Q(X)$ lies in $\mathcal{S}^\perp$. Moreover, for any morphism $f \colon X \to Y$ we obtain an exact sequence
...
\begin{topic}{tensor-triangulated-category}{tensor triangulated category}
    A \textbf{tensor triangulated category} is a \tref{triangulated-category}{triangulated category} $\mathcal{T}$ with a \tref{CT:symmetric-monoidal-category}{symmetric monoidal structure} $\otimes \colon \mathcal{T} \times \mathcal{T} \to \mathcal{T}$ such that $X \otimes (-)$ and $(-) \otimes X$ are \tref{exact-functor-triangulated}{exact functors} for all $X$ in $\mathcal{T}$.
\end{topic}
...
\begin{topic}{t-structure}{t-structure}
    ...
    The \textbf{heart} of the $t$-structure is the full subcategory $\mathcal{D}^\heartsuit = \mathcal{D}_{\ge 0} \cap \mathcal{D}_{\le 0}$, and is \tref{abelian-category}{abelian}.
...
\begin{topic}{tilting-object}{tilting object}
    Let $\mathcal{T}$ be a \tref{triangulated-category}{triangulated category}. An object $X$ of $\mathcal{T}$ is \textbf{tilting} if
    \begin{itemize}
        \item $X$ generates $\mathcal{T}$ as triangulated category,
        \item $\Hom_\mathcal{T}(X, X[\ell]) = 0$ for $\ell \ne 0$,
        \item the category of $\Hom_{\mathcal{T}}(X, X)$-modules has finite \tref{homological-dimension}{global dimension}.
    \end{itemize}
\end{topic}

\begin{example}{tilting-object}
    If $E_1, \ldots, E_n$ is a \tref{exceptional-sequence}{full strong exceptional sequence} then $X = \bigoplus_{i = 1}^{n} E_i$ is a tilting object. Namely, $X$ generates $\mathcal{T}$ as $E_1, \ldots, E_n$ generate $\mathcal{T}$, and
