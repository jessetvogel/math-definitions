...
\begin{topic}{universal-enveloping-algebra}{universal enveloping algebra}
    ...
    It has the universal property that for any \tref{algebra}{algebra} $A$ and Lie algebra morphism $\varphi \colon \mathfrak{g} \to A$, where the Lie bracket on $A$ is given by the commutator, there exists a unique algebra morphism $U(\mathfrak{g}) \to A$ such that
...
\begin{topic}{poisson-algebra}{Poisson algebra}
    A \textbf{Poisson algebra} is a commutative \tref{algebra}{algebra} $A$ over a \tref{field}{field} $k$, together with an operation $\{ \cdot, \cdot \} \colon A \otimes_k A \to A$, called the \textbf{Poisson bracket}, making $A$ into a \tref{lie-algebra}{Lie algebra} and satisfying the \textit{Leibniz rule} $\{ f, gh \} = \{ f, g \} h + g \{ f, h \}$. % $\{ a, - \} \colon A \to A$ is a \tref{derivation}{$k$-derivation} for every $a \in A$.
\end{topic}
...
\begin{topic}{root-system-lie-algebra}{root system Lie algebra}
    Let $\mathfrak{g}$ be a complex \tref{simple-lie-algebra}{semisimple} \tref{lie-algebra}{Lie algebra}, and $\mathfrak{h}$ a \tref{cartan-subalgebra}{Cartan subalgebra}. A \textbf{root} of $\mathfrak{g}$ (relative to $\mathfrak{h}$) is an element $\alpha \in \mathfrak{h}^*$ such that
...
\begin{topic}{killing-form}{Killing form}
    ...
    where $\textup{ad}_\mathfrak{g} \colon \mathfrak{g} \to \mathfrak{gl}(\mathfrak{g})$ is the \tref{DG:adjoint-representation}{adjoint representation} of $\mathfrak{g}$.
\end{topic}
...
\begin{topic}{lie-theorem}{Lie's theorem}
    Let $\mathfrak{g}$ be a \tref{solvable-lie-algebra}{solvable} complex \tref{lie-algebra}{Lie algebra}, and $\pi \colon \mathfrak{g} \to \mathfrak{gl}(V)$ a finite-dimensional representation. Then \textbf{Lie's theorem} states that there exists a basis for $V$ in which $\pi(\mathfrak{g})$ consists of upper triangular matrices.
\end{topic}
...
\begin{example}{radical-lie-algebra}
    \begin{itemize}
        ...
        \item The radical of a \tref{simple-lie-algebra}{semisimple} Lie algebra $\mathfrak{g}$ is zero.
    \end{itemize}
\end{example}

\begin{topic}{dynkin-diagram}{Dynkin diagram}
    Let $A = (a_{ij})$ be an $n \times n$ \tref{LA:cartan-matrix}{Cartan matrix}, often coming from a \tref{simple-lie-algebra}{semisimple Lie algebra}. The \textbf{Dynkin diagram} $D(A)$ of $A$ is the graph, consisting of $n$ vertices, such that the edge between vertex $i$ and $j$ is determined by the $2 \times 2$ submatrix $\begin{pmatrix} a_{ii} & a_{ij} \\ a_{ji} & a_{jj} \end{pmatrix}$ according to the following rule:
...
\begin{topic}{verma-module}{Verma module}
    ...
    The Verma module $V_\lambda$ has the universal property that for any \tref{lie-algebra-representation}{representation} $\pi \colon \mathfrak{g} \to \mathfrak{gl}(V)$ generated by a highest weight vector of weight $\lambda$, there is a surjective map $V_\lambda \to V$ of representations.
\end{topic}
...
\begin{topic}{casimir-element}{Casimir element}
    Let $\mathfrak{g}$ be a \tref{simple-lie-algebra}{semisimple Lie algebra}. For any basis $e_1, \ldots, e_n$ of $\mathfrak{g}$ with dual basis $e^1, \ldots, e^n$ with respect to some non-degenerate bilinear form $B \colon \mathfrak{g} \times \mathfrak{g} \to k$ (commonly the \tref{killing-form}{Killing form}), the \textbf{Casimir element} of $\mathfrak{g}$ is the element
...
\begin{topic}{dg-lie-algebra}{differential graded Lie algebra}
    A \textbf{differential graded Lie algebra (dgla)} over a \tref{field}{field} $k$ is a \tref{HA:chain-complex}{chain complex} $(\mathfrak{g}_\bdot, d)$ of $k$-modules with a bilinear map $[\cdot, \cdot] \colon \mathfrak{g}_p \otimes \mathfrak{g}_q \to \mathfrak{g}_{p + q}$ satisfying
    \begin{itemize}
...
\begin{example}{dg-lie-algebra}
    ...
    % This yields a (forgetful) functor $\textbf{Alg}^\textup{dg}_k \to \textbf{Lie}^\textup{dg}_k$, which has a \tref{CT:adjunction}{left adjoint} $U \colon \textbf{Lie}^\textup{dg}_k \to \textbf{Alg}^\textup{dg}_k$, which assigns to every dgla $\mathfrak{g}_\bdot$ its \textit{universal enveloping algebra} 
...
\begin{topic}{heisenberg-lie-algebra}{Heisenberg Lie algebra}
    Let $V$ be a \tref{LA:vector-space}{vector space} over a \tref{field}{field} $k$, and let $\omega \colon V \times V \to k$ be a skew-symmetric bilinear form on $V$. The \textbf{Heisenberg Lie algebra} corresponding to $(V, \omega)$ is the \tref{lie-algebra}{Lie algebra} $V \oplus k$ whose Lie bracket is given by
    \[ [ (v, \alpha), (w, \beta) ] = (0, \omega(v, w)) . \]
\end{topic}

\begin{topic}{trace-form-lie-algebra}{trace form Lie algebra}
    Let $\mathfrak{g}$ be a \tref{lie-algebra}{Lie algebra} with \tref{lie-algebra-representation}{representation} $\pi \colon \mathfrak{g} \to \mathfrak{gl}(V)$. The \textbf{trace form} on $\mathfrak{g}$ corresponding to $\pi$ is the symmetric bilinear form on $\mathfrak{g}$ given by
    \[ (x, y) \mapsto \tr (\pi(x) \pi(y)) . \]
\end{topic}

\begin{example}{trace-form-lie-algebra}
    Provided $\mathfrak{g}$ is finite-dimensional, the trace form on $\mathfrak{g}$ corresponding to the \tref{DG:adjoint-representation}{adjoint representation} $\textup{ad}_\mathfrak{g} \colon \mathfrak{g} \to \mathfrak{gl}(\mathfrak{g})$ is the \tref{killing-form}{Killing form}.
\end{example}
