...
\begin{example}{algebraic-transcendental}
    ...
    % \[ A \colon k(\alpha) \to k(\alpha), \quad x \mapsto \alpha x \qquad \textup{ and } \qquad B \colon k(\beta) \to k(\beta), \quad x \mapsto \beta x . \]
...
\begin{topic}{prime-field}{prime field}
    The \textbf{prime field} of a \tref{field}{field} $k$ is its smallest subfield. When $\textup{char}(k) = 0$, this is $\QQ$ and when $\textup{char}(k) = p > 0$, this is $\mathbb{F}_p$.
\end{topic}

\begin{topic}{frobenius-morphism}{Frobenius morphism}
    Let $k$ be a \tref{field}{field} of characteristic $p > 0$. The map $F \colon k \to k : x \mapsto x^p$ is a homomorphism (use binomial theorem) called the \textbf{Frobenius morphism}.
...
\begin{topic}{purely-inseparable-field-extension}{purely inseparable field extension}
    An \tref{algebraic-field-extension}{algebraic} \tref{field-extension}{field extension} $\ell/k$ is \textbf{purely inseparable} if for every $\alpha \in \ell \setminus k$, the \tref{minimal-polynomial}{minimal polynomial} of $\alpha$ over $k$ is not \tref{separable-polynomial}{separable}.
\end{topic}
...
\begin{topic}{separably-closed-field}{separably closed field}
    A \tref{field}{field} $k$ is \textbf{separably closed} if every \tref{separable-field-extension}{separable field extension} $\ell/k$ is trivial, that is, $\ell = k$.
\end{topic}
...
\begin{example}{galois-extension}
    \begin{itemize}
        \item The field extension $\FF_2[\sqrt{t}] \supset \FF_2[t]$ is normal, since any field extension of degree $2$ is normal, but not separable. Namely, the minimal polynomial of $\sqrt{t}$ is $x^2 - t = (x - \sqrt{t})^2$, which is not separable.
        \item The field extension $\QQ(\sqrt[3]{2}) \supset \QQ$ is separable, since any field extension over characteristic zero is separable, but not normal. Namely, the conjugates of $\sqrt[3]{2}$ are $\zeta_3 \sqrt[3]{2}$ and $\zeta_3^2 \sqrt[3]{2}$, which do not lie in $\QQ(\sqrt[3]{2})$.
        \item Its normal closure, $\QQ(\sqrt[3]{2}, \zeta_3)$, is normal, and hence a Galois extension of $\QQ$.
    \end{itemize}
\end{example}
...
\begin{topic}{galois-group}{Galois group}
    The \textbf{Galois group} $\textup{Gal}(\ell/k)$ of a \tref{galois-extension}{Galois extension} $\ell \supset k$ is the group of $\ell$-linear automorphisms of $\ell$, that is $\textup{Aut}_k(\ell)$.
\end{topic}

\begin{topic}{galois-correspondence}{Galois correspondence}
    Let $\ell/k$ be a \tref{galois-extension}{Galois extension} with \tref{galois-group}{Galois group} $G = \textup{Gal}(\ell/k)$. The \textbf{Galois correspondence} states there is an inclusion-reversing bijection between the set of intermediate fields $k \subset m \subset \ell$ and the set of subgroups of $G$.
    \[ \svg \begin{tikzcd}[row sep=0em]
...
\begin{example}{zariski-lemma}
    \begin{proof}
        ...
        \[ \alpha \colon k[x_1, \ldots, x_n] \to \ell \]
...
\begin{topic}{retract-rational-field}{retract rational field}
    Let $k$ be a \tref{field}{field}. A \tref{field-extension}{field extension} $K/k$ is \textbf{retract rational} if there exists a $k$-algebra $A$ in $K$, whose \tref{field-of-fractions}{field of fractions} is $K$, a non-zero polynomial $f \in k[x_1, \ldots, x_n]$, and $k$-algebra morphisms $\varphi \colon A \to k[x_1, \ldots, x_n, f^{-1}]$ and $\psi : k[x_1, \ldots, x_n, f^{-1}]$ such that $\psi \circ \phi = \id_A$.
\end{topic}
