...
%     A \textbf{differential graded algebra} $A$ is a graded algebra with a map $d \colon A \to A$ having degree $1$ (cochain complex convention) or $-1$ (chain complex convention) satisfying
%     \begin{itemize}
...
%     with differential $d \colon \Hom_n(A, B) \to \Hom_{n + 1}(A, B)$ such that $d^2 = 0$. Furthermore, composition of morphisms
...
%     A \textbf{graded Lie algebra} is a graded vector space $\mathfrak{g} = \bigoplus_{i \in \ZZ} \mathfrak{g}^i$ with a bilinear map $[\cdot, \cdot] \colon \mathfrak{g}^i \otimes \mathfrak{g}^j \to \mathfrak{g}^{i + j}$
% \end{topic}

% \begin{topic}{differential-graded-lie-algebra}{differential graded Lie algebra}
%     A \textbf{differential graded Lie algebra} is a \tref{graded-lie-algebra}{graded Lie algebra} $\mathfrak{g}$  and a differential, that is, a linear operator $d \colon \mathfrak{g} \to \mathfrak{g}$ of degree $+1$, satisfying the Leibniz rule
...
\begin{topic}{dg-algebra}{(commutative) differential graded algebra}
    A \textbf{differential graded algebra (dga)} over a \tref{field}{field} $k$ is a \tref{HA:chain-complex}{chain complex} $(A_\bdot, d)$ of $k$-modules with an associative multiplication map $A_i \times A_j \to A_{i + j}$, satisfying the Leibniz rule
