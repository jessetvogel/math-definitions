...
\begin{topic}{matrix-factorization}{matrix factorization}
    Let $R$ be a \tref{ring}{ring} and $x$ an element of $R$. A \textbf{matrix factorization} of $x$ is a pair of \tref{free-module}{free} $R$-modules $M$ and $N$, together with $R$-module morphisms $f : M \to N$ and $g : N \to M$ such that $g \circ f = x \cdot \id_M$ and $f \circ g = x \cdot \id_N$.
\end{topic}
...
\begin{topic}{skolem-noether-theorem}{Skolem--Noether theorem}
    Let $A$ be a \tref{simple-ring}{simple} $k$-algebra and $B$ be a \tref{finite-algebra}{finite} \tref{central-simple-algebra}{central simple} $k$-algebra, over a \tref{field}{field} $k$. The \textbf{Skolem--Noether theorem} states that, if $f, g : A \to B$ are two $k$-algebra morphisms, then there exists an invertible element $b \in B$ such that
    \[ f(a) = b g(a) b^{-1} \]
    for all $a \in A$.
\end{topic}
