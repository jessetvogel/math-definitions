\begin{topic}{monoid}{monoid}
    A \textbf{monoid} is a set $M$ with a binary operation $\cdot \colon M \times M \to M$ satisfying
    \begin{itemize}
...
\begin{topic}{semigroup}{semigroup}
    A \textbf{semigroup} is a set $S$ with a binary operation $\cdot \colon S \times S \to S$ which is associative, that is, $(a \cdot b) \cdot c = a \cdot (b \cdot c)$ for all $a, b, c \in S$.
\end{topic}

\begin{topic}{semiring}{semiring}
    A \textbf{semiring} is a set $R$ with two binary operations $+ \colon R \times R \to R$, called addition, satisfying
    \begin{itemize}
    ...
    \end{itemize}
    and $\cdot \colon R \times R \to R$, called multiplication, satisfying
    \begin{itemize}
...
\begin{topic}{bialgebra}{bialgebra}
    A \textbf{bialgebra} over a \tref{field}{field} $k$ is an \tref{algebra}{algebra} $A$ over $k$ together with morphisms of $k$-algebras $\Delta \colon A \to A \otimes A$ (the \textit{comultiplication}) and $\varepsilon \colon A \to k$ (the \textit{counit}), satisfying
    \begin{itemize}
...
\begin{topic}{hopf-algebra}{Hopf algebra}
    A \textbf{Hopf algebra} over a \tref{field}{field} $k$ is a \tref{bialgebra}{bialgebra} $A$ over $k$ together with a $k$-linear map $S \colon A \to A$ (the \textit{antipode}), such that the diagram
    \[ \svg \begin{tikzcd}[column sep=1em, row sep=2em] & A \otimes A \arrow{rr}{S \otimes \id} && A \otimes A \arrow{dr}{\mu} & \\ A \arrow{rr}{\varepsilon} \arrow{ur}{\Delta} \arrow[swap]{dr}{\Delta} && k \arrow{rr}{\eta} && A \\ & A \otimes A \arrow[swap]{rr}{\id \otimes S} && A \otimes A \arrow[swap]{ur}{\mu} &  \end{tikzcd} \]
...
\begin{topic}{group-like-element}{group-like element}
    ...
    \[ \Delta(a) = a \otimes a , \]
    where $\Delta \colon A \to A \otimes A$ denotes the comultiplication map.
\end{topic}
...
\begin{example}{group-like-element}
    ...
    \[ \chi \colon G \to \GG_m = \Spec(k[t, t^{-1}]), \quad t \mapsto a , \]
...
%     with multiplication $\Delta^* \colon A^\circ \otimes_k A^\circ \to A^\circ$, unit $\varepsilon^* \colon k \to A^\circ$, comultiplication $\mu^* \colon A^\circ \to A^\circ \otimes_k A^\circ$, counit $\eta^* \colon A^\circ \to k$ and antipode $S^* \colon A^\circ \to A^\circ$.
% \end{topic}

\begin{topic}{frobenius-algebra}{Frobenius algebra}
    A \textbf{Frobenius algebra} over a \tref{field}{field} $k$ is a \tref{algebra}{$k$-algebra} $A$ equipped with a bilinear form $\beta \colon A \otimes_k A \to k$, called the \textit{Frobenius pairing}, satisfying
    \begin{itemize}
        \item (\textit{associativity}) $\beta(ab \otimes c) = \beta(a \otimes bc)$ for all $a, b, c \in A$,
        \item (\textit{non-degeneracy}) there exists a $k$-linear map $\gamma \colon k \to A \otimes_k A$ such that 
...
\begin{example}{frobenius-algebra}
    \begin{itemize}
        \item Let $A$ be a finite field extension of $k$, and $\varepsilon \colon A \to k$ any non-zero $k$-linear map. Then $A$ with the pairing $\beta \colon A \otimes_k A \to k, a \otimes b \mapsto \varepsilon(ab)$ is a Frobenius algebra.
        ...
        \item The complex numbers $\CC$ are a Frobenius algebra over $\RR$ with pairing $\beta(z_1 \otimes z_2) = \operatorname{Re}(z_1 z_2)$. The map $\gamma \colon \RR \to \CC \otimes_\RR \CC$ is given by $\gamma(1) = 1 \otimes 1 - i \otimes i$.
        \item The \tref{RT:representation-ring}{representation ring} $R_\CC(G)$ of a finite \tref{GT:group}{group} $G$ is a Frobenius algebra over $\CC$, where multiplication is given by convolution
        \[ (a * b)(g) = \sum_{h \in G} a(h) b(h^{-1} g) \quad \textup{ for } a, b \in R_\CC(G) \]
        and the bilinear form $\beta$ is given by the usual \tref{RT:first-orthogonality-theorem}{inner product}
        \[ \beta(a \otimes b) = \frac{1}{|G|} \sum_{g \in G} a(g) b(g^{-1}) \quad \textup{ for } a, b \in R_\CC(G) . \]
        The map $\gamma$ is given by $\gamma(1) = \sum_{\chi \in \hat{G}} \chi \otimes \chi$ where $\hat{G}$ denotes the set of \tref{RT:irreducible-representation}{irreducible representations} of $G$.
    \end{itemize}
\end{example}
...
\begin{example}{frobenius-algebra}
    ...
    \[ \varepsilon \colon A \to k, \quad a \mapsto \beta(1 \otimes a) = \beta(a \otimes 1) \]
    which is well-defined by associativity of $\beta$. As comultiplication, take
    \[ \delta \colon A \to A \otimes_k A, \quad a \mapsto (\mu \otimes \id_A)(a \otimes \gamma(1)) = (\id_A \otimes \mu)(\gamma(1) \otimes a) . \]
...
\begin{topic}{augmentation}{augmentation}
    An \textbf{augmentation} of an \tref{algebra}{algebra} over a field $k$, is a morphism of $k$-algebras $\varepsilon \colon A \to k$.
\end{topic}
...
\begin{topic}{hochschild-homology}{Hochschild (co)homology}
    Let $k$ be a \tref{field}{field}, $A$ an \tref{algebra}{$k$-algebra}, and $M$ a \tref{bimodule}{bimodule} over $A$. Let $C_n(A, M) := M \otimes A^{\otimes n}$, and define boundary maps $d_{n, i} \colon C_n(A, M) \to C_{n - 1}(A, M)$ for $0 \le i \le n$ by
...
\begin{example}{hochschild-homology}
    The (first order) deformations of a ring $A$ up to trivial deformations are parametrized by $HH^2(A)$. Namely, any deformed multiplication $a \star b := ab + \varepsilon f(a, b)$ with $f \colon A \otimes A \to A$ and $\varepsilon^2 = 0$ is associative if and only if $(a \star b) \star c = a \star (b \star c)$ for all $a, b, c \in A$. This translates to
...
\begin{topic}{gerstenhaber-algebra}{Gerstenhaber algebra}
    A \textbf{Gerstenhaber algebra} is a \tref{graded-ring}{graded} \tref{algebra}{$k$-algebra} $A = \bigoplus_{n \in \ZZ} A^n$, equipped with a bilinear map $[-, -] \colon A \times A \to A$ of degree $-1$, such that
    \begin{itemize}
...
\begin{topic}{noether-normalization-lemma}{Noether's normalization lemma}
    Let $A$ be a \tref{finitely-generated-algebra}{finitely generated algebra} over a \tref{field}{field} $k$. \textbf{Noether's normalization lemma} states that there exists an integer $n \ge 0$ and an injective morphism of $k$-algebras
    \[ k[x_1, \ldots, x_n] \to A , \]
...
\begin{topic}{huber-pair}{Huber pair}
    ...
    A morphism of Huber pairs $(A, A^+) \to (B, B^+)$ is a continuous ring morphism $f \colon A \to B$ such that $f(A^+) \subset B^+$.
\end{topic}

\begin{topic}{matrix-factorization}{matrix factorization}
    Let $R$ be a \tref{ring}{ring} and $x$ an element of $R$. A \textbf{matrix factorization} of $x$ is a pair of \tref{free-module}{free} $R$-modules $M$ and $N$, together with $R$-module morphisms $f \colon M \to N$ and $g \colon N \to M$ such that $g \circ f = x \cdot \id_M$ and $f \circ g = x \cdot \id_N$.
\end{topic}
...
\begin{topic}{puiseux-series}{Puiseux series}
    Let $k$ be a \tref{field}{field}. A \textbf{Puiseux series} with coefficients in $k$ is an expression of the form
...
\begin{topic}{hecke-algebra}{Hecke algebra}
    Let $(W, S)$ be a finite \tref{GT:coxeter-group}{Coxeter group}. For a complex number $q \in \CC$, the \textbf{$q$-Hecke algebra} $\mathcal{H}_q(W, S)$ of $(W, S)$ is the \tref{algebra}{$\CC$-algebra} generated by $\{ T_{s_i} \}_{s_i \in S}$ subject to the relations
    \[ T_{s_i}^2 = (q - 1) T_{s_i} + q \quad \textup{ and } \quad (T_{s_i} T_{s_j})^{m(s_i, s_j)} = 1 \textup{ for } i \ne j . \]
\end{topic}

\begin{example}{hecke-algebra}
    Let $G = \operatorname{GL}_2(\FF_q)$ with \tref{AG:borel-subgroup}{Borel subgroup} $B = \left\{ \left( \begin{smallmatrix} x & y \\ 0 & z \end{smallmatrix} \right) \right\} \subset G$. Consider the $\CC$-algebra
    \[ \mathcal{H}(G, B) = \{ f \colon G \to \CC \mid f(b_1 z b_2) = f(z) \textup{ for all } b_1, b_2 \in B \} \]
    where multiplication is given by convolution
    \[ (f * g)(z) = \frac{1}{|B|} \sum_{\substack{x, y \in G \\ \textup{s.t.\ } xy = z}} f(x) g(y) . \]
    The \tref{AG:bruhat-decomposition}{Bruhat decomposition} identifies the set of double cosets $B \backslash G / B$ with the Weyl group $W \cong S_2 = \langle w \rangle$ of $G$. Hence, we can identify $\mathcal{H}(G, B) \cong \CC \cdot T_1 \oplus \CC \cdot T_2$, where
...
\begin{topic}{double-centralizer-theorem}{double centralizer theorem}
    Let $k$ be a \tref{field}{field} and $V$ a finite-dimensional \tref{LA:vector-space}{vector space} over $k$. The \textbf{double centralizer theorem} states that for any semisimple $k$-subalgebra $A$ of $\operatorname{End}_k(V)$,
...
\begin{topic}{finite-algebra}{finite algebra}
    Let $R$ be a \tref{ring}{commutative ring}. An \tref{algebra}{$R$-algebra} $A$ is \textbf{finite} if it is \tref{finitely-generated-module}{finitely generated} as $R$-module.
\end{topic}

\begin{topic}{skolem-noether-theorem}{Skolem--Noether theorem}
    Let $A$ be a \tref{simple-ring}{simple} $k$-algebra and $B$ be a \tref{finite-algebra}{finite} \tref{central-simple-algebra}{central simple} $k$-algebra, over a \tref{field}{field} $k$. The \textbf{Skolem--Noether theorem} states that, if $f, g \colon A \to B$ are two $k$-algebra morphisms, then there exists an invertible element $b \in B$ such that
    \[ f(a) = b g(a) b^{-1} \]
    for all $a \in A$.
\end{topic}
