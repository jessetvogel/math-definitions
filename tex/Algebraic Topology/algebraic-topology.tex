\begin{topic}{singular-homology}{singular (co)homology}
    Let $X$ be a \tref{TO:topological-space}{topological space} and $A$ an \tref{GT:abelian-group}{abelian group}. A \textbf{singular $n$-simplex} in $X$ is a \tref{TO:continuous-map}{continuous map} $\sigma \colon \Delta^n \to X$, where 
    ...
    and the boundary maps $\partial_n \colon C_n(X; A) \to C_{n - 1}(X; A)$ are induced from
    \[ \partial_n \colon \mathcal{S}(X)_n \to \mathcal{S}(X)_{n - 1}, \quad \sigma \mapsto \sum_{i = 1}^{n} (-1)^i \, \sigma \circ \delta_i , \]
    where $\delta_i \colon \Delta^{n - 1} \to \Delta^n$ maps to the $i$-th face.
...
\begin{topic}{fundamental-group}{fundamental group}
    ...
    \[ \pi_1(X, x_0) = \{ \text{continuous } f \colon [0, 1] \to X : f(0) = x_0 = f(1) \} / \sim{} , \]
...
\begin{example}{mayer-vietoris-sequence}
    ...
    where $\Delta \colon A \oplus A \to A$ is given by $(a, b) \mapsto a + b$. Now we can read off that $H^0(X; A) = \ker(\Delta) \isom A$ and $H^1(X; A) \isom A \oplus A$ and $H^n(X; A) = 0$ for $n \ge 2$.
\end{example}
...
\begin{topic}{cup-product}{cup product}
    ...
    where $i_p^\text{front} \colon \Delta^{p} \to \Delta^{p + q}$ sends vertex $i$ to vertex $i$, and $i_q^\text{back}$ sends vertex $i$ to vertex $n - q + i$.
    
    The cup product descents to a product on the cohomology groups
    \[ (-) \smile (-) \colon H^p(X; R) \times H^q(X; R) \to H^{p + q}(X; R), \qquad ([f], [g]) \mapsto [f \smile g] , \]
    ...
    \begin{itemize}
        ...
        \item (\textit{functorial}) for every map $\varphi \colon X \to Y$, $\varphi^*(f \smile g) = \varphi^* f \smile \varphi^* g$,
...
\begin{topic}{cap-product}{cap product}
    ...
    \[ (-) \frown (-) \colon C^q(X; R) \times C_{p + q}(X; R) \to C_p(X; R) \]
    induced by sending $f \colon \mathcal{S}(X)_q \to R$ and $\sigma \in \mathcal{S}(X)_{p + q}$ to
    \[ f \frown \sigma = (-1)^{pq} f(\sigma \circ i_q^\text{back}) \cdot (\sigma \circ i_p^\text{front})  \]
    where $i_p^\text{front} \colon \Delta^{p} \to \Delta^{p + q}$ sends vertex $i$ to vertex $i$, and $i_q^\text{back}$ sends vertex $i$ to vertex $n - q + i$.
    
    The cup product descents to a product on the cohomology groups
    \[ (-) \frown (-) \colon H^q(X; R) \times H_{p + q}(X; R) \to H_q(X; R), \qquad ([f], [g]) \mapsto [f \frown g] . \]
\end{topic}

\begin{topic}{homotopy}{homotopy}
    A \textbf{homotopy} between \tref{TO:continuous-map}{continuous maps} $f, g \colon X \to Y$ is a continuous map
...
\begin{topic}{homotopy-equivalence}{homotopy equivalence}
    A \textbf{homotopy equivalence} between two \tref{TO:topological-space}{topological spaces} $X$ and $Y$ is a pair of \tref{TO:continuous-map}{continuous maps} $f \colon X \to Y$ and $g \colon Y \to X$ such that $g \circ f$ is \tref{homotopy}{homotopic} to $\id_X$ and $f \circ g$ is homotopic to $\id_Y$. If such a pair exists, then $X$ and $Y$ are said to be \textbf{homotopy equivalent}.
\end{topic}

\begin{example}{homotopy-equivalence}
    The $n$-sphere $S^n \subset \RR^{n + 1}$ is homotopic to $\RR^{n + 1} \setminus \{ 0 \}$. Namely, consider the inclusion $i \colon S^n \to \RR^{n + 1}$ and the map
    \[ f \colon \RR^{n + 1} \setminus \{ 0 \} \to S^n, \quad x \mapsto \frac{x}{\|x\|} . \]
    Then $f \circ i = \id_{S^n}$, and $i \circ f$ is homotopic to $\id_{\RR^{n + 1}}$ since
    \[ H \colon \RR^{n + 1} \times [0, 1] \to \RR^{n + 1}, \quad (x, t) \mapsto (1 - t) x + \frac{tx}{\|x\|} \]
    satisfies $H(-, 0) = i \circ f$ and $H(-, 1) = \id_{\RR^{n + 1}}$.
\end{example}

\begin{topic}{weak-homotopy-equivalence}{weak homotopy equivalence}
    A \textbf{weak homotopy equivalence} is a continuous map $f \colon X \to Y$ if it induces bijections
    \[ f_* \colon \pi_n(X, x) \to \pi_n(Y, f(x)) \]
    for all $n \ge 0$ and all basepoints $x \in X$.
\end{topic}

\begin{topic}{null-homotopic}{null homotopic}
    A continuous map $f \colon X \to Y$ between \tref{TO:topological-space}{topological spaces} is \textbf{null-homotopic} if it is homotopic to a constant map.
\end{topic}
...
\begin{topic}{cohomology-compact-support}{cohomology with compact support}
    ...
    \[ C_c^n(X; R) = \big\{ f \colon \mathcal{S}(X)_n \to R : \text{ there is a compact $K \subset X$ with $f(\sigma) = 0$ for all $\sigma \in \mathcal{S}(X \setminus K)_n$} \big\} . \]
...
\begin{topic}{homotopy-extension-property}{homotopy extension property}
    Let $X$ be a \tref{TO:topological-space}{topological space} and $A \subset X$ a subspace. Then the pair $(X, A)$ has the \textbf{homotopy extension property} (HEP) if for any topological space $Z$, continuous map $f \colon X \to Z$ and homotopy $F \colon A \times [0, 1] \to Z$ with $F|_{A \times \{ 0 \}} = f|_A$, there exists a homotopy $H \colon X \times [0, 1] \to Z$ with $H|_{X \times \{ 0 \}} = f$ and $H|_{A \times [0, 1]} = F$.
...
\begin{topic}{homotopy-lifting-property}{homotopy lifting property}
    Let $p \colon E \to B$ be a \tref{TO:continuous-map}{continuous map}, and let $A \subset X$ be a pair of topological spaces. Then $p$ is said to have the \textbf{homotopy lifting property} (HLP) for $(X, A)$ if for any continuous map $f \colon X \to E$, homotopy $F \colon A \times [0, 1] \to E$ with $F|_{A \times \{ 0 \}} = f|_A$, and homotopy $G \colon X \times [0, 1] \to B$ with $G|_{X \times \{ 0 \}} = p \circ f$ and $G|_{A \times [0, 1]} = p \circ F$, there exists a homotopy $H \colon X \times [0, 1] \to E$ such that $H|_{A \times [0, 1]} = F$, $H|_{X \times \{ 0 \}} = f$ and $p \circ H = G$.
    \[ \svg \begin{tikzcd}[column sep=4em]
...
\begin{topic}{hurewicz-fibration}{Hurewicz fibration}
    A \tref{TO:continuous-map}{continuous map} $p \colon E \to B$ is a \textbf{Hurewicz fibration} if it has the \tref{homotopy-lifting-property}{homotopy lifting property} with respect to all topological spaces $X$.
\end{topic}

\begin{example}{hurewicz-fibration}
    Any map of the form $\pi_B \colon B \times F \to B$ is a Hurewicz fibration. Namely, for any $f \colon X \to B \times F$ and $G \colon X \times [0, 1] \to B$ with $G|_{X \times \{ 0 \}} = f$ we can simply take $H \colon X \times [0, 1] \to B \times F$ given by $H(x, t) = (G(x, t), \pi_F(f(x)))$.
\end{example}

\begin{topic}{serre-fibration}{Serre fibration}
    A \tref{TO:continuous-map}{continuous map} $p \colon E \to B$ is a \textbf{Serre fibration} if it has the \tref{homotopy-lifting-property}{homotopy lifting property} with respect to all $n$-disks $D^n$ for $n \ge 0$.
...
\begin{topic}{cellular-map}{cellular map}
    A map $f \colon (X, A) \to (Y, B)$ of \tref{cw-complex}{relative CW-complexes} is called \textbf{cellular} if $f(X_n) \subset Y_n$ for all $n \ge -1$. Here $X_n$ denotes the $n$-skeleton of $X$.
\end{topic}

\begin{topic}{cellular-approximation}{cellular approximation}
    The \textbf{cellular approximation theorem} states that every map $f \colon (X, A) \to (Y, B)$ of \tref{cw-complex}{relative CW-complexes} is \tref{homotopy}{homotopic} relative to $A$ to a \tref{cellular-map}{cellular map}.
    
    That is, there exists a continuous map $H \colon X \times [0, 1] \to Y$ such that $H(x, 0) = f(x)$ for all $x \in X$, the map $x \mapsto = H(x, 1)$ is cellular, and $H(a, t) = f(a)$ for all $a \in A$ and $t \in [0, 1]$.
\end{topic}

\begin{topic}{whitehead-theorem}{Whitehead theorem}
    The \textbf{Whitehead theorem} states that any \tref{weak-homotopy-equivalence}{weak homotopy equivalence} $f \colon X \to Y$ between \tref{cw-complex}{CW-complexes} is a \tref{homotopy-equivalence}{homotopy equivalence}.
\end{topic}
...
\begin{topic}{homotopy-fiber}{homotopy fiber}
    Let $f \colon X \to Y$ be a \tref{TO:continuous-map}{continuous map} of pointed spaces $X, Y$ with basepoints $x \in X$ and $y \in Y$. Then the \textbf{homotopy fiber} of $f$ is the space
...
\begin{example}{homotopy-fiber}
    The map $f \colon X \to Y$ can be factorized as
...
\begin{topic}{homotopy-cofiber}{homotopy cofiber}
    Let $f \colon X \to Y$ be a \tref{TO:continuous-map}{continuous map} of pointed spaces $X, Y$ with basepoints $x \in X$ and $y \in Y$. Then the \textbf{homotopy cofiber} of $f$ is the space
    \[ C(f) = Y \cup_X CX , \]
...
\begin{topic}{cofibration}{cofibration}
    A map $i \colon A \to X$ of \tref{TO:topological-space}{topological spaces} is a \textbf{cofibration} if for any space $Y$ and map $f$ as in the diagram
    \[ \svg \begin{tikzcd} X \times \{ 0 \} \cup A \times [0, 1] \arrow{r}{f} \arrow{d} & Y \\ X \times [0, 1] \arrow[dashed]{ur}{g} \end{tikzcd} \]
    there exists an extension $g \colon X \times [0, 1] \to Y$. That is, the map $i$ satisfies the \tref{homotopy-extension-property}{homotopy extension property}.
\end{topic}
...
\begin{topic}{generalized-cohomology-theory}{generalized cohomology theory}
    A \textbf{generalized cohomology theory} is a sequence of contravariant \tref{CT:functor}{functors} $h^i$, indexed by $i \in \ZZ$, from the category of \tref{cw-complex}{CW-pairs} $(X, A)$ to the category of \tref{GT:abelian-group}{abelian groups}, together with a \tref{CT:natural-transformation}{natural transformation} $d^i \colon h^i(A) \to h^{i + 1}(X, A)$ called the \textit{boundary morphism} (writing $h^i(A)$ for $h^i(A, \varnothing)$). These should satisfy the following axioms:
    \begin{itemize}
        \item (\textit{homotopy}) \tref{homotopy}{homotopic} maps induce the same map on cohomology,
        \item (\textit{exactness}) each pair $(X, A)$ induces a long exact sequence, via the inclusions $i \colon A \to X$ and $j \colon (X, \varnothing) \to (X, A)$,
        \[ \cdots \rightarrow h^i(X, A) \xrightarrow{j_*} h^i(X) \xrightarrow{i_*} h^i(A) \xrightarrow{d} h^{i + 1}(X, A) \rightarrow \cdots \]
        \item (\textit{excision}) if $X$ is the union of subcomplexes $A$ and $B$, then the inclusion $f \colon (A, A \cap B) \to (X, B)$ induces isomorphisms
...
\begin{topic}{cellular-homology}{cellular homology}
    Let $X$ be a \tref{cw-complex}{CW-complex}, and write $X_n$ for the $n$-skeleton, $J_n$ for the indexing set of $n$-cells, and $f_n^i \colon \partial D^n \to X_{n - 1}$ for the attaching map of cell $i \in J_n$. The \textbf{cellular complex} $\tilde{C}_\bdot(X; A)$ of $X$ with coefficients in an \tref{GT:abelian-group}{abelian group} $A$ is given by the \tref{relative-homology}{relative homology groups}
    \[ \tilde{C}_n(X; A) = H_n(X_n, X_{n - 1}; A) , \]
    with differentials $\tilde{\partial}_n \colon \tilde{C}_n(X; A) \to \tilde{C}_{n - 1}(X; A)$ given by the connecting homomorphism of the long exact sequence of the triple $X_{n - 2} \subset X_{n - 1} \subset X_n$. The \textbf{cellular homology} of $X$ with coefficients in $A$ is then given
    ...
    where $e_n^i$ denotes the generator corresponding to the $n$-cell $i \in J_n$, and $\chi_n^{ij} \colon S^{n - 1} \to S^{n - 1}$ denotes the composition
...
\begin{topic}{brouwer-fixed-point-theorem}{Brouwer fixed point theorem}
    Let $f \colon D^n \to D^n$ be a \tref{TO:continuous-map}{continuous map} where $D^n = \{ x \in \RR^n \mid \norm{x} \le 1 \}$ is the $n$-disk. Then \textbf{Brouwer's fixed point theorem} states that there exists some $x \in D^n$ with $f(x) = x$.
\end{topic}

\begin{example}{brouwer-fixed-point-theorem}
    \begin{proof}
        Suppose for a contradiction that $f(x) \ne x$ for all $x \in D^n$. Then there exists a unique line from $f(x)$ to $x$, and we let $r(x) \in \partial D^n$ be the point where this line intersects the boundary of $D^n$ (closer to $x$ than to $f(x)$). The resulting map $r \colon D^n \to \partial D^n$ can be shown to be continuous, and since clearly $r(x) = x$ for all $x \in \partial D^n$, the map $r$ is a \tref{CT:retraction}{retraction} of the inclusion $i \colon \partial D^n \to D^n$, that is, $r \circ i = \id_{\partial D^n}$. Looking at \tref{singular-homology}{singular homology} $H_{n - 1}(-; \ZZ)$, we find that the composition
        \[ H_{n - 1}(\partial D^n; \ZZ) \xrightarrow{i_*} H_{n - 1}(D^n; \ZZ) \xrightarrow{r_*} H_{n - 1}(\partial D^n; \ZZ) , \]
        must equal the identity. However, $H_{n - 1}(\partial D^n; \ZZ) = H_{n - 1}(S^{n - 1}; \ZZ) \isom \ZZ$ and $H_{n - 1}(D^n; \ZZ) = 0$, and clearly the identity $\id \colon \ZZ \to \ZZ$ cannot factor through $0$, so we have a contradiction.
    \end{proof}
\end{example}

\begin{topic}{lefschetz-fixed-point-theorem}{Lefschetz fixed point theorem}
    Let $X$ be a \tref{cw-complex}{finite CW-complex}, and $f \colon X \to X$ be a \tref{TO:continuous-map}{continuous map}. The \textbf{Lefschetz number} of $f$ is defined as
    \[ \Lambda_f = \sum_{k \ge 0} (-1)^k \tr \left(f_* \colon H_k(X; \QQ) \to H_k(X; \QQ) \right) , \]
...
\begin{example}{lefschetz-fixed-point-theorem}
    \tref{brouwer-fixed-point-theorem}{Brouwer's fixed point theorem} follows from Lefschetz fixed point theorem. Namely, let $f \colon D^n \to D^n$ be a continuous map. Note that $H_0(D^n; \QQ) = \QQ$ and $H_i(D^n; \QQ) = 0$ for all $i > 0$. Since $f_* \colon H_0(D^n; \QQ) \to H_0(D^n; \QQ)$ is the identity map, it follows that $\Lambda_f = 1 \ne 0$, so $f$ has a fixed point.
\end{example}
...
\begin{topic}{universal-bundle}{universal bundle}
    Let $G$ be a \tref{TO:topological-group}{topological group}. A \textbf{universal $G$-bundle} is a \tref{TO:principal-bundle}{principal $G$-bundle} $\pi \colon EG \to BG$ whose total space $EG$ is \tref{contractible-space}{contractible}.
...
\begin{topic}{thom-gysin-sequence}{Thom--Gysin sequence}
    Let $R$ be a \tref{AA:ring}{commutative ring}. Let $\pi \colon E \to B$ be a \tref{serre-fibration}{Serre fibration}  with fiber $S^n$, where $B$ is a \tref{TO:simply-connected-space}{simply connected} \tref{cw-complex}{CW-complex}. The \textbf{Thom--Gysin sequence} is the long exact sequence of homology groups
...
\begin{topic}{intersection-homology}{intersection homology}
    ...
    with $X_{m - 1} = X_{m - 2}$, such that for all $x \in X_k \setminus X_{k - 1}$, there exists a neighborhood $N_x$ of $x$ in $X$, a compact topologically pseudomanifold $L = L_{m - k - 1} \supset \cdots \supset L_0$ of dimension $m - k - 1$, and a homeomorphism $\phi \colon N_x \to \RR^k \times C(L)$ (with $C(L)$ the open cone of $L$) such that \textit{(i)} $\phi$ takes $N_x \cap X_{k + i + 1}$ homeomorphically to $\RR^k \times C(L_i)$ for $0 \le i \le m - k - 1$, and \textit{(ii)} $\phi$ takes $N_x \cap X_k$ homeomorphically to $\RR^k \times \{ \textup{vertex} \}$.

    A \textbf{perversity} is a function $p \colon \ZZ_{\ge 2} \to \ZZ$ such that $p(2) = 0$ and $p(k + 1) - p(k) \in \{ 0, 1 \}$ for all $k \ge 2$.

    Given a perversity $p$, a \tref{singular-homology}{singular $i$-simplex} $\sigma \colon \Delta^i \to X$ is \textbf{$p$-allowable} if $\sigma^{-1}(X_{m - k} \setminus X_{m - k - 1})$ is contained in the $(i - k + p(k))$-skeleton of $\Delta^i$ for all $k \ge 2$. A singular $i$-chain $\xi = \sum_j a_j \sigma_j$ is \textbf{$p$-allowable} if all singular $i$-simplices (with non-zero coefficient) appearing in $\xi$ or $\partial \xi$ are $p$-allowable.
