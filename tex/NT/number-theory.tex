...
\begin{topic}{picard-group}{Picard group}
    Let $R$ be a \tref{CA:domain}{domain} with \tref{CA:field-of-fractions}{field of fractions} $K$. Let $\mathcal{I}(R)$ be the set of \tref{CA:invertible-ideal}{invertible ideals} of $R$, which forms a group under multiplication. The set of principal \tref{CA:fractional-ideal}{fractional ideals} forms a subgroup $\mathcal{P}(R) \subset \mathcal{I}(R)$, isomorphic to $K^* / R^*$. The \textbf{Picard group} of $R$ is the quotient
...
\begin{topic}{discriminant}{discriminant}
    Let $f \in R[X]$ be a polynomial over some \tref{CA:domain}{domain} $R$. Then $f$ can be written as
...
\begin{example}{abel-ruffini-theorem}
    \textbf{Proof}. Let $f \in k[x]$ be a polynomial over a field $k$, and let $\ell/k$ be a \tref{CA:splitting-field}{splitting field} of $f$. Then in particular $\ell/k$ is a \tref{CA:galois-extension}{Galois extension}. Now if $f$ is solvable by radicals, then we can write
    \[ k = \ell_0 \subset \ell_1 \subset \ell_2 \subset \cdots \subset \ell_m = \ell , \]
    where each $\ell_{i + 1}/\ell_i$ is a simple radical extension, that is, $\ell_{i + 1} = \ell_i(\sqrt[n_i]{\alpha_i})$ for some $n_i \in \NN$ and $\alpha_i \in \ell_i$. This implies the Galois groups $\textup{Gal}(\ell_{i + 1}/\ell_i) \simeq \ZZ/n_i\ZZ$ are all \tref{GT:cyclic-group}{cyclic}, and via the \tref{CA:galois-correspondence}{Galois correspondence} we obtain
...
\begin{topic}{multiplier-ring}{multiplier ring}
    Let $R$ be a \tref{CA:domain}{domain}, $K$ its \tref{CA:field-of-fractions}{field of fractions}, and $I$ a \tref{CA:fractional-ideal}{fractional ideal} of $R$. The \textbf{multiplier ring} of $I$ is the subring of $K$ given by
