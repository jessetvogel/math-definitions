...
\begin{topic}{alexandroff-topology}{Alexandroff topology}
    Let $(P, \le)$ be a \tref{ST:partially-ordered-set}{partially ordered set}. The \textbf{Alexandroff topology} on $P$ is the \tref{topological-space}{topology} on $P$ given by
...
\begin{topic}{stone-cech-compactification}{Stone--Čech compactification}
    Let $X$ be a \tref{topological-space}{topological space}. The \textbf{Stone--Čech compactification} of $X$ is a \tref{compact-space}{compact} \tref{hausdorff-space}{Hausdorff} space $\beta(X)$ together with a \tref{continuous-map}{continuous map} $i : X \to \beta(X)$, such that for any continuous map $f : X \to K$, with $K$ a compact Hausdorff space, there exists a unique $g : \beta(X) \to K$ such that $f = g \circ i$.
