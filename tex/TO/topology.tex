...
\begin{topic}{T4-space}{T4 space}
    A \textbf{T4 space} is a \tref{topological-space}{topological space} which is \tref{hausdorff-space}{Hausdorff} and \tref{normal-space}{normal}.
\end{topic}
...
\begin{topic}{paracompact-space}{paracompact space}
    A \tref{topological-space}{topological space} $X$ is \textbf{paracompact} if every open cover of $X$ has a refinement that is locally finite. An open cover is \textit{locally finite} if any point has an open neighborhood where only finitely many sets of the open cover intersect.
\end{topic}
