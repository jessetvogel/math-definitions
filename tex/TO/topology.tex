...
\begin{topic}{first-countable-space}{first countable space}
    A \tref{topological-space}{topological space} $X$ is \textbf{first countable} if for every point $x \in X$, there exists a sequence of \tref{neighborhood}{neighborhoods} $N_1, N_2, \ldots$ of $x$ such that for every neighborhood $N$ of $x$ contains some $N_i$.
\end{topic}
...
\begin{topic}{sequentially-compact-space}{sequentially compact space}
    A \tref{topological-space}{topological space} is \textbf{sequentially compact} if every sequence of points $(x_n)_{n \in \NN}$ in $X$ has a \tref{convergent-sequence}{convergent} subsequence.
\end{topic}
...
\begin{topic}{separable-space}{separable space}
    A \tref{topological-space}{topological space} $X$ is \textbf{separable} if it contains a countable \tref{dense}{dense} subset.
\end{topic}
...
\begin{topic}{baire-space}{Baire space}
    ...
    Equivalently, $X$ is a Baire space if the intersection of a countable number of \tref{dense}{dense} subsets of $X$ is again dense.
\end{topic}
...
\begin{topic}{alexandroff-topology}{Alexandroff topology}
    Let $(P, \le)$ be a \tref{OT:partially-ordered-set}{partially ordered set}. The \textbf{Alexandroff topology} on $P$ is the \tref{topological-space}{topology} on $P$ given by
...
\begin{topic}{regular-space}{regular space}
    A \tref{topological-space}{topological space} $X$ is \textbf{regular} if for every closed set $Y \subset X$ and every point $x \in X \setminus Y$, there exist disjoint open subsets $U, V \subset X$ such that $Y \subset U$ and $p \in V$.
\end{topic}
