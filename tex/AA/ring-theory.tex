...
\begin{topic}{algebra}{algebra}
    Let $R$ be a \tref{ring}{commutative ring}. An \textbf{$R$-algebra} is ring $A$ with a \tref{ring-morphism}{ring morphism} $f : R \to A$.
\end{topic}

\begin{example}{algebra}
    Let $R = k$ be a field, and $A = \textup{Mat}_{n}(k)$ the ring of $n \times n$ matrices over $k$. Then $A$ is a $k$-algebra under the morphism $i : k \to A$ which sends $x \mapsto x \cdot I$.
\end{example}
