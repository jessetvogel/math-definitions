...
\begin{topic}{local-ring}{local ring}
    A \textbf{local ring} is a \tref{ring}{commutative ring} $R$ with exactly one \tref{maximal-ideal}{maximal ideal}. The maximal is often denoted by $\mathfrak{m}$.
...
\begin{topic}{discrete-valuation-ring}{discrete valuation ring}
    ...
    In particular, $R$ is \tref{local-ring}{local} with maximal ideal $\mathfrak{m} = \{ x \in K \mid v(x) > 0 \}$. Indeed, any element $x \in R$ with $v(x) = 0$ is a unit in $R$.
\end{topic}
